...
LOG_CONCLUSION_RE = re.compile(
    r"^(CONFIRMED|NEEDS_VERIFICATION|OPEN)\s+\S"
)
LOG_CAP = 8
PATH_TRIM = "`'\"（）()「」[]【】<>"
...
def parse_log(body, context_text=""):
    ...
    ctx_cites = _parse_cites(_strip_html_comments(context_text))
    entries = []
    i = 0
    while i < len(raw_lines):
        ...
        if not LOG_CONCLUSION_RE.match(conclusion):
            raise ValueError(
