#!/bin/bash
# 版本同步守衛:Gauntlet 版本在四處必須是**同一個值**。
#
# 四處(缺一或值不同即 FAIL;不是只 grep「版本字串存在」,是逐處抽值後比較):
...
for label in ("gauntlet-script", "contract-json", "readme", "design-doc"):
    print(f"  • {label}: {found.get(label, '(抽不到)')}")

if len(found) == 4:
    values = set(found.values())
    if len(values) == 1:
        print(f"  ✓ 四處一致:{next(iter(values))}")
    else:
        problems.append(f"四處版本不一致:{found}")
