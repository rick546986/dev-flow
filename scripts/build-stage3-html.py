#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""第 3 站審頁產生器 —— 從 3-prototype.md 產出觸發判定／Demo／回寫 2-decision。

契約:notes/design/stage3-review-ui-contract.md
牙:scripts/check-stage3-proto-contract.sh

這支只產第 3 站給人審的頁。本站選配:feat 沒有 3-prototype.md 時
n-a／跳過,不要 exit 1 當硬缺。有檔才印頁。
不進 build-gate-twin.py STAGES。不包 markdown-it + html-shell。
不要把 html-shell 舊樣當正式。卡殼優先;至少自足單檔、深淺色。
不發明產品規則。

用法:
  scripts/build-stage3-html.py <3-prototype.md|feat目錄> [--out PATH]
  scripts/build-stage3-html.py --action <3-prototype.md|feat目錄> [--out PATH]
  scripts/build-stage3-html.py --fixture

--fixture 把 scripts/fixtures/stage3-html/proto-page.md 印到 stdout。
給檔時預設寫同目錄 3-prototype.html(或 --out)。
給 feat 目錄且沒有 3-prototype.md → 印 n-a,exit 0,不寫 html。

exit:0 = 寫出或 n-a / 1 = 有檔但解析不到觸發判定 / 2 = 用法錯誤
"""
...
FIXTURE = ROOT / "scripts" / "fixtures" / "stage3-html" / "proto-page.md"

FM_RE = re.compile(r"\A---\n(.*?)\n---\n", re.S)
H2_RE = re.compile(r"^##[ \t]+(.+?)\s*$", re.M)
H3_RE = re.compile(r"^###[ \t]+(.+?)\s*$", re.M)
CHECK_RE = re.compile(r"^[-*]\s*\[([ xX])\]\s*(.+)$", re.M)
...
.trigger{list-style:none;padding:0;margin:0}
.trigger li{padding:4px 0;border-bottom:1px solid var(--rule-2)}
...
def usage():
    print(
        "用法:build-stage3-html.py <3-prototype.md|feat目錄> [--out PATH]\n"
        "     build-stage3-html.py --action <3-prototype.md|feat目錄> [--out PATH]\n"
        "     build-stage3-html.py --fixture\n"
        "契約:notes/design/stage3-review-ui-contract.md\n"
        "產檔器吐觸發判定／Demo／答案回寫 2-decision。沒有 3-prototype.md 就 n-a。\n"
        "不進 build-gate-twin.py STAGES,不包 markdown-it + html-shell。",
        file=sys.stderr,
    )
...
def parse_triggers(body):
    items = []
    for mark, raw in CHECK_RE.findall(body or ""):
        items.append((mark.lower() == "x", raw.strip()))
    return items


def parse_demos(body):
    found = list(H3_RE.finditer(body or ""))
    demos = []
    for i, match in enumerate(found):
        title = match.group(1).strip()
        stop = found[i + 1].start() if i + 1 < len(found) else len(body)
        blob = body[match.end():stop]
        fields = []
        for item in LIST_RE.findall(blob):
            fields.append(item.strip())
        if not fields:
            prose = first_sentence(blob)
            if prose:
                fields.append(prose)
        demos.append((title, fields))
    if not demos:
        prose = first_sentence(body)
        if prose:
            demos.append(("Demo", [prose]))
    return demos


def parse_steps(fig_body):
    steps = []
    for item in LIST_RE.findall(fig_body or ""):
        steps.append(re.sub(r"^\[.?\]\s*", "", item.strip()))
    if not steps:
...
def render_vbox(steps):
    ...
    parts = [
        '<svg class="vbox" viewBox="0 0 %d %d" role="img" aria-label="結構圖">'
        % (CANVAS_W, height),
    ]
    y = TOP
    for i, title in enumerate(steps):
        kind = "hl" if ("選定" in title or i == 0) else "b"
        parts.append(
...
def render_block(block_id, title, inner):
    return (
        '<section class="r-block" id="%s">'
        '<div class="r-head"><span class="r-name">%s</span></div>'
        '<div class="r-body">%s</div></section>'
    ) % (esc(block_id), esc(title), inner)


def human_verdict(feedback):
    match = re.search(r"Human verdict:\s*(\S+)", feedback or "")
    if match:
...
def build_html(text):
    ...
    slug = meta.get("feature") or "3-prototype"
    status = meta.get("status") or ""
    _, trig_body = section_named(body, ("Stage 3 觸發判定", "觸發判定"))
    triggers = parse_triggers(trig_body)
    if not triggers:
        die(1, "解析不到 Stage 3 觸發判定")
    hits = sum(1 for hit, _t in triggers if hit)
    _, question = section_named(body, ("Question",))
    _, demo_body = section_named(body, ("Demo Script", "Demo"))
    demos = parse_demos(demo_body)
    if not demos:
        die(1, "解析不到 Demo Script")
    _, result = section_named(body, ("Result", "答案"))
    _, verdict = section_named(body, ("Verdict",))
    _, feedback = section_named(body, ("User Demo Feedback",))
    _, fig_body = section_named(body, ("結構圖",))
    steps = parse_steps(fig_body)
    hv = human_verdict(feedback)
    writeback = first_sentence(verdict)
    if "2-decision" not in (writeback + first_sentence(result)):
        writeback = (writeback + " 回寫 2-decision。").strip()

    trig_items = []
    for hit, label in triggers:
        cls = "hit" if hit else "miss"
        mark = "命中" if hit else "未中"
        trig_items.append(
            '<li class="%s">[%s] %s</li>' % (cls, mark, esc(label))
        )
    demo_cards = []
    for title, fields in demos:
        bits = "".join("<p>%s</p>" % esc(f) for f in fields) or "<p>—</p>"
        demo_cards.append('<div class="card"><strong>%s</strong>%s</div>' % (esc(title), bits))

    fig_html = ""
    if steps:
        fig_html = (
            '<div class="figwrap">\n%s\n</div>' % render_vbox(steps)
        )

    dash = (
        '<div class="dash">'
        '<div class="cell"><span class="k">狀態</span><span class="v">%s</span></div>'
        '<div class="cell"><span class="k">觸發命中</span><span class="v">%d/%d</span></div>'
        '<div class="cell"><span class="k">Demo</span><span class="v">%d</span></div>'
        '<div class="cell"><span class="k">Human verdict</span><span class="v">%s</span></div>'
        "</div>"
    ) % (
        esc(status or "draft"),
        hits,
...
<title>%s · 3-prototype</title>
<style>%s</style>
</head>
<body>
<div class="wrap">
<header class="masthead">
  <p class="eyebrow">dev-flow · 3-prototype · 審查介面</p>
  <h1>%s</h1>
  <p class="sub">正本是 md。觸發判定／Demo／答案回寫 2-decision。不是 html-shell 長文。</p>
  %s
</header>
%s
%s
...
%s
%s
<p class="src">由 <code>scripts/build-stage3-html.py</code> 從 md 解析產生,不包 html-shell。</p>
</div>
</body>
</html>
""" % (
        ...
        render_block(
            "trigger",
            "觸發判定",
            "<ul class=\"trigger\">%s</ul>" % "".join(trig_items),
        ),
        render_block("question", "Question", "<p>%s</p>" % esc(first_sentence(question) or "—")),
        render_block("demo", "Demo", fig_html + "".join(demo_cards)),
        render_block("result", "答案", "<p>%s</p>" % esc(first_sentence(result) or "—")),
        render_block(
            "writeback",
            "回寫 2-decision",
            "<p>%s</p>" % esc(writeback or "回寫 2-decision"),
        ),
        render_block(
            "feedback",
            "User Demo Feedback",
            "<p>%s</p>" % esc(first_sentence(feedback) or "—"),
        ),
    )
    return page
...
def resolve_md(src):
    ...
    if path.is_dir():
        cand = path / "3-prototype.md"
        if cand.is_file():
            return cand, False
        return cand, True
    if path.name == "3-prototype.md" and not path.is_file():
        return path, True
    return path, False


def emit_na():
    sys.stdout.write("n-a: no 3-prototype.md\n")
