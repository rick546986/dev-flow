...
check_static_pin "scripts/check-py-floor.sh" "MIN_HEREDOCS = 229" "MIN_HEREDOCS 釘死 229(精確實測;baseline 221 + guard Read INTERP-only +1 → 222;2026-09-23 jev-gate W2 test-devflow-jev.sh ②⑤ 治具 +2、selftest p3 ship-manifest 版本寫入 +3 → 227、同日審查補穿越案 +2 → 229;RW-DG1/2 改 tag MUT 不進掃描集;關掉 INTERP 必跌破,不靠抬地板)"
check_static_pin "scripts/check-file-map.sh" "EXPECTED_MAPPED_FILES = 231" "EXPECTED_MAPPED_FILES 釘死 231(精確值;knowledge-index +2、proof +1、knife-2 ask +2、knowledge bootstrap +2、check-preload-ban.sh +1、diagram-ir-gate +3、five-station-f1 +3、five-station-f2 +2、five-station-f3 +3、jev-gate W1 守衛套件 scripts/devflow_jev/ 11 .py(jev-questions.json 不計)+ test-devflow-jev.sh +12 → 225、P2-1 test-spec-gate-e2e.sh +1 → 226、jev-gate W2 runtime devflow-jev.py + http_transport/state + test_runtime/test_http_transport +5 → 231)"
