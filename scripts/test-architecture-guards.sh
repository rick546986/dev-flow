...
#                    的 EXPECTED_GROUPS(含 28 個群組名逐字整行釘)/
...
check_static_pin "scripts/check-dev-setup-discipline.sh" "MIN_CHECKS = 23" "MIN_CHECKS 釘死 23(A-2/B-5 輪:②改 scoped 拆 3 條 + ⑦⑧⑨ 新增 → 15;2026-08-20 ⑩check 段散發副本 parity map-driven 拆 3 條 → 18;2026-08-28 ⑪upgrade 殘件刪除 +5 → 23)"
...
check_static_pin "scripts/check-file-map.sh" "EXPECTED_MAPPED_FILES = 181" "EXPECTED_MAPPED_FILES 釘死 181(精確值,不是地板;2026-08-28 upgrade 殘件刪除加 devflow-upgrade-leftovers.sh + test-upgrade-leftovers.sh,疊在三邊 plugin 薄殼牙 179 之上)"
...
echo "  ✓ guard-selfpin 兩條斷言字面 + 28 個群組名逐字整行釘全過(X-3 HIGH-1/MED,二次複審後改為整行釘)"
echo
