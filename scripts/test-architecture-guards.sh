...
#                    的 EXPECTED_GROUPS(含 29 個群組名逐字整行釘)/
...
p = pathlib.Path(sys.argv[1]) / "docs/dev/readme-contract-extract.md"
t = p.read_text(encoding="utf-8")
n = t.replace("devflow-evidence-gauntlet.sh`(1.4.0,", "devflow-evidence-gauntlet.sh`(9.9.9,", 1)
...
t = p.read_text(encoding="utf-8")
n = t.replace('"gauntlet": "1.4.0"', '"gauntlet": "9.9.9"', 1)
...
t = p.read_text(encoding="utf-8")
n = t.replace("**現行 Gauntlet 版本:1.4.0**", "**現行 Gauntlet 版本:9.9.9**", 1)
...
p = pathlib.Path(sys.argv[1]) / "notes/design/evidence-gauntlet.md"
t = p.read_text(encoding="utf-8")
n = t.replace("**現行 Gauntlet 版本:1.4.0**\n", "", 1)
...
t = p.read_text(encoding="utf-8")
n = t.replace('GAUNTLET_VERSION="1.4.0"', 'GAUNTLET_VERSION="9.9.9"', 1)
...
check_static_pin "scripts/check-ship-manifest.sh" "MIN_CHECKS = 26" "MIN_CHECKS 釘死 26(結構+parity+地圖對帳+負向 fixture 的實得數;issue #92 補全列 source 存在 +2、第三類列負向 fixture +4,15→21;2026-09-23 W2 C1 manifest.version↔契約 ship_manifest_version 正向 +1、版本負向 fixture +3 → 25;同日審查 反向掃 tools/ 子目錄負向 +1 → 26)"
check_static_pin "scripts/check-status-policy.sh" "MIN_CHECKS = 55" "MIN_CHECKS 釘死 55(STATUS 單寫入者 + OverlapRef 單一座標:⑬b/⑬c + 負向㉘–㉟ 後的實得數)"
check_static_pin "scripts/check-py-floor.sh" "MIN_HEREDOCS = 230" "MIN_HEREDOCS 釘死 230(精確實測;baseline 221 + guard Read INTERP-only +1 → 222;2026-09-23 jev-gate W2 test-devflow-jev.sh ②⑤ 治具 +2、selftest p3 ship-manifest 版本寫入 +3 → 227、同日審查補穿越案 +2 → 229;jev-gate W6 check-verdict-attestation.sh +1 → 230;RW-DG1/2 改 tag MUT 不進掃描集;關掉 INTERP 必跌破,不靠抬地板)"
check_static_pin "scripts/check-file-map.sh" "EXPECTED_MAPPED_FILES = 232" "EXPECTED_MAPPED_FILES 釘死 232(精確值;knowledge-index +2、proof +1、knife-2 ask +2、knowledge bootstrap +2、check-preload-ban.sh +1、diagram-ir-gate +3、five-station-f1 +3、five-station-f2 +2、five-station-f3 +3、jev-gate W1 守衛套件 scripts/devflow_jev/ 11 .py(jev-questions.json 不計)+ test-devflow-jev.sh +12 → 225、P2-1 test-spec-gate-e2e.sh +1 → 226、jev-gate W2 runtime devflow-jev.py + http_transport/state + test_runtime/test_http_transport +5 → 231、jev-gate W6 check-verdict-attestation.sh +1 → 232)"
...
# 逐一釘死 29 個群組名——清單住在本檔(誰要改名/刪名都得同步這裡,否則這裡會現形;
...
REQUIRED_GROUP_NAMES=(
  ...
  "decision-review-shape" "tasks-review-shape" "g3-review-shape" "guard-selfpin"
  "fig-ascii-191"
)
...
echo "  ✓ guard-selfpin 兩條斷言字面 + 29 個群組名逐字整行釘全過(X-3 HIGH-1/MED,二次複審後改為整行釘)"
echo
