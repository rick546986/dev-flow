...
check_static_pin "scripts/check-ship-manifest.sh" "MIN_CHECKS = 25" "MIN_CHECKS 釘死 25(結構+parity+地圖對帳+負向 fixture 的實得數;issue #92 補全列 source 存在 +2、第三類列負向 fixture +4,15→21;2026-09-23 W2 C1 manifest.version↔契約 ship_manifest_version 正向 +1、版本負向 fixture +3 → 25)"
check_static_pin "scripts/check-status-policy.sh" "MIN_CHECKS = 55" "MIN_CHECKS 釘死 55(STATUS 單寫入者 + OverlapRef 單一座標:⑬b/⑬c + 負向㉘–㉟ 後的實得數)"
check_static_pin "scripts/check-py-floor.sh" "MIN_HEREDOCS = 227" "MIN_HEREDOCS 釘死 227(精確實測;baseline 221 + guard Read INTERP-only +1 → 222;2026-09-23 jev-gate W2 test-devflow-jev.sh ②⑤ 治具 +2、selftest p3 ship-manifest 版本寫入 +3 → 227;RW-DG1/2 改 tag MUT 不進掃描集;關掉 INTERP 必跌破,不靠抬地板)"
