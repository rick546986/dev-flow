...
## Context(已知事實)
<!-- 每條 = 已驗證斷言 + 出處。出處只有 path:L<起> 或 path:L<起>-L<迄>;
...
## Interview Log(推理鏈外顯)
<!-- 每條巢狀四段。子項剛好 2 個空白縮排(不是 tab、不是 4 空白)。
     標籤只能是 事實／推理／結論(不接受 事實依據)。
     事實欄只引用 Context 已列出處;結論以 CONFIRMED／NEEDS_VERIFICATION／OPEN 開頭。
     層1 盤現況確認的事實、層3 what-if 推演與盲點戰果也記於此 -->
- Q:…
  - 事實:docs/specs/<domain>.md:L1-L2
  - 推理:…
  - 結論:CONFIRMED …
