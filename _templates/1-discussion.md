...
### Assumption 四欄
<!-- 高影響假設就地或本表填:若為假影響什麼／影響級／怎麼驗／何時／由誰驗 -->
| 若為假影響什麼 | 影響級 | 怎麼驗 | 何時／由誰驗 |
|---|---|---|---|

## Evidence manifest
<!-- 與本檔同檔,不另造永久 manifest。owner 核准 ∈ {是, 未核, 禁}。
     核准=是才能在 talk 游標在時 Read 該擬路徑;是不能覆寫 2–7 方案檔禁令。 -->
...
## Requested solution
<!-- 解法構想住這裡,並標未定案。dashboard／API／元件通道不是 Goal。 -->
...
<!-- 每條 Goal 翻成 1-3 條可驗證敘述(假設…當…則…),使用者視角、不綁實作方式。
     ...
     - AC-1:假設<前置>,當<動作>,則<結果>
       - 從哪看:<從哪裡看出結果發生;log | 產出檔 | 人可核對的紀錄。不預填畫面／API 通道>
