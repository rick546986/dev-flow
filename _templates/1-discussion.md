...
<!-- 每條 = 已驗證斷言 + 出處。出處語法只有一種:path:L<起> 或 path:L<起>-L<迄>;
     ...
     驗證 = 讀過該路徑、該行段支持斷言。brief／ask() 只是線索,未驗證不進此節。 -->
...
## Interview Log(推理鏈外顯)
<!-- 每條巢狀四段。子項正好兩格縮排(不是 tab、不是 4 空白)。
     標籤固定為 事實／推理／結論(事實依據 不接受)。
     事實欄只引用 Context 已列出處,至少一個 path:L<起> 或 path:L<起>-L<迄>。
     結論以 CONFIRMED／NEEDS_VERIFICATION／OPEN 開頭。高影響在 Q 前標 ⚠️。 -->
- Q:…
  - 事實:…
  - 推理:…
  - 結論:CONFIRMED …
