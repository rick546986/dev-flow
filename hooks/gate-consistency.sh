#!/bin/bash
# gate-consistency.sh — gate 條件摘要 vs 契約檔 §7 正本一致性檢查(手動跑,不掛 hook)。
# 從母版 docs/dev/readme-contract-extract.md §7(G1/G2/G3 唯一正本)動態抽取每個 gate 的關鍵 token,比對三處摘要
# (plugin dev-flow SKILL.md 階段表 / 契約檔 §3 表 / 三份模板頂註送審步)是否都含該
