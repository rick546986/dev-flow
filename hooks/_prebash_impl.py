...
def _devstage3_graph_action(command):
    """Stage 3 graph:本機有 .devstage3-cursor.json 才編成 3-prototype / 2-decision / 4-spec。

    必須在 load_state 早退之前跑。沒游標檔不攔截 —— 不是沙盒。
    不重做第 1 站 write_code 編成。不改第 2 站 write_spec 編成。
    """
    import tempfile

    cursor_path = os.path.join(root, ".devstage3-cursor.json")
    if not os.path.isfile(cursor_path):
        return
    action = None
    if _writes_named_md(command, "3-prototype.md"):
        action = "write_prototype"
    elif _writes_named_md(command, "2-decision.md"):
        action = "write_decision"
    elif _writes_named_md(command, "4-spec.md"):
        action = "write_spec"
    if not action:
    ...
    if not isinstance(cursor, dict):
        cursor = {}
    check = os.path.join(root, "scripts", "check-devstage3-graph.sh")
    if not os.path.isfile(check):
        return
    payload_path = None
    try:
    ...
    if proc.returncode != 0:
        reason = (proc.stdout or proc.stderr or "action denied").strip()
        _obs_deny("devstage3-graph", action)
...
def _devstage6_graph_action(command):
    """Stage 6 graph:本機有 .devstage6-cursor.json 才編成 6-notes。

    必須在 load_state 早退之前跑。沒游標檔不攔截 —— 不是沙盒。
    不重做第 1 站 write_code 編成。不改第 2／3／4／5 站的編成 —— 那四站各認自己的
    游標檔,五份游標同時在場時各自送自己的 --action(第 5 站也編成
    6-implementation-notes.md,但它讀的是 .devstage5-cursor.json,兩邊不互相拆掉)。
    只編成 6-implementation-notes.md → write_notes。不編成 5-tasks.md／4-spec.md。
    """
    import tempfile

    cursor_path = os.path.join(root, ".devstage6-cursor.json")
    if not os.path.isfile(cursor_path):
        return
    if not _writes_named_md(command, "6-implementation-notes.md"):
        return
    action = "write_notes"
    try:
        cursor = json.load(open(cursor_path, encoding="utf-8"))
    ...
    if not isinstance(cursor, dict):
        cursor = {}
    check = os.path.join(root, "scripts", "check-devstage6-graph.sh")
    if not os.path.isfile(check):
        return
    payload_path = None
    try:
    ...
    if proc.returncode != 0:
        reason = (proc.stdout or proc.stderr or "action denied").strip()
        _obs_deny("devstage6-graph", action)
        L.die("⛔ Stage 6 graph --action deny:" + reason)


def _devstage7_graph_action(command):
    """Stage 7 graph:本機有 .devstage7-cursor.json 才編成 7-review。

    必須在 load_state 早退之前跑。沒游標檔不攔截 —— 不是沙盒。
    不重做第 1 站 write_code 編成。不改第 2／3／4／5／6 站的編成 —— 那五站各認自己的
    游標檔,六份游標同時在場時各自送自己的 --action。
    只編成 7-review.md → write_review。不編成 4-spec.md／5-tasks.md／
    6-implementation-notes.md。不改鬆圍欄③(N4 才讀 6-notes 仍由 --action 裁決)。
    """
    import tempfile

    cursor_path = os.path.join(root, ".devstage7-cursor.json")
    if not os.path.isfile(cursor_path):
        return
    if not _writes_named_md(command, "7-review.md"):
        return
    action = "write_review"
    try:
        cursor = json.load(open(cursor_path, encoding="utf-8"))
    ...
    if not isinstance(cursor, dict):
        cursor = {}
    check = os.path.join(root, "scripts", "check-devstage7-graph.sh")
    if not os.path.isfile(check):
        return
    payload_path = None
    try:
    ...
    if proc.returncode != 0:
        reason = (proc.stdout or proc.stderr or "action denied").strip()
        _obs_deny("devstage7-graph", action)
        L.die("⛔ Stage 7 graph --action deny:" + reason)
