...
# 狀態:.devflow/exec.json(scope/baseline+hash/契約 hash;task 模式 = exec-v2)+
...
case "$CMD" in
  ...
  candidate|parallel-init|plan|wave-open|wave-close|task-candidate|task-state|task-integrate|task-rework|rebuild-plan|review)
