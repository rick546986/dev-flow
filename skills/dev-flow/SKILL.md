...
| 6 實作 | **`dev-run` 引擎**(層與升階見指南 `#flow`;Cursor／Grok 預設 Auto,見 `skills/dev-run/SKILL.md` 主機分流;守衛 `devflow-exec.sh` start/stop,詳其 SKILL;5-tasks 明寫 `execution.mode: parallel` 時走並行引擎(選配))或手動逐 T;兩者共用指南 `#stage6`／契約檔 §5 的 T acceptance seam:RED→GREEN→scope check→Verify→獨立 T review→PASS→commit→記 Progress Log/checkbox/review evidence;**執行清單見 `_templates/6-implementation-notes.md` 頂註**;節點鏈見 `skills/dev-flow/stage6/` | `6-implementation-notes.md`(含 T Review Log;執行軌跡只供 dev-run) | 每 T review PASS + 全 S 綠 |
