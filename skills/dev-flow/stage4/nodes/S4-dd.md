...
兩層分級判準與「依據」欄的寫法正本在 `_templates/4-spec.md` Drafting Decisions
...
跑 `${CLAUDE_PLUGIN_ROOT}/scripts/check-devstage4-graph.sh --write-cursor S4-dd`。

## 完成條件

掃描零殘留,每條 DD 的「依據」欄寫得出出處或標 `[Assumption]`。
只有一份 `4-spec.md`。本機游標在 S4-dd。
