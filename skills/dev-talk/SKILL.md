...
1. **盤現況**。正本:`nodes/S1-survey.md`。入口摘要:消化 brief,讀白名單條列事實含受影響面;認可確認理解,不是來源升格。完成條件見該節點。
...
3. **逐題逼問**(循環)。正本:`nodes/N3-probe.md`。入口摘要:一次只問一題;
   **發現｜**禁附推薦,**裁決｜**可附選項／差異／推薦;覆述推理鏈、清單外斷言必問;
   3a 核銷五份時同步改現況圖;其餘子條款見該節點。完成條件見該節點。
4. **驗收雛形**。正本:`nodes/S4-accept.md`。入口摘要:Goal 翻成假設…當…則…並問從哪裡看出結果發生。完成條件見該節點。
