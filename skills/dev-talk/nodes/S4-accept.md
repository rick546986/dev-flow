...
不綁做法),每條再問出「怎麼看到」三件:①從哪裡看出結果發生
②看到什麼算對(具體值、狀態變化、可重開的產出)③拿什麼試(現成的真實資料或
情境;沒有就問要不要造一筆)。說不出結果在哪發生 = 這條其實還沒定義清楚,回步 3 問。
問現象不問做法 —— 「從哪裡看出結果發生」可問,「該用什麼元件/回什麼 JSON」不問。
畫面／端點／元件不是 Goal 本身;通道構想寫進 Requested solution 並標未定案。
逐條請使用者確認。
