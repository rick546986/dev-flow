...
**Human PASS**（2026-08-28）。七張 named shots 已收。
...
| S-id | 測試 | 狀態 |
|---|---|---|
| S-1.1 | 靜態 `ivf.php:7553-7557` 未滿40第1次 100000／0／50000。畫面未跑 | unverified |
| S-1.2 | 靜態 `ivf.php:7578-7590` 3.0 未滿39第2次 60000／40000；非 PLUS 不 keep。畫面未跑 | unverified |
| S-1.3 | 靜態 `ivf.php:7558-7561` 第2–3次 60000／50000／40000。畫面未跑 | unverified |
| S-2.1 | 同 S-1.3；`ivf_subsidy_edit.php:2721-2726` PLUS 三格解 readonly。畫面未跑 | unverified |
| S-2.2 | 靜態 happy path：手改重算 limit、keep POST、再開 derive。畫面未跑 | unverified |
| S-2.3 | 抽驗：`controllers/backend/ivf.php:7568-7571` 滿40第1次 70000／10000／50000 | unverified |
...
| grep T-2 三格 | `rg -n "胚胎形成" views/backend/ivf_subsidy_edit.php` | pass | 3 處 | |
...
|---|---|---|---|
| S-1.1 | 附表五三格＋上限。申請日填 9/1 | 已拍 plus-two-cells／v30-two-cells。靜態：`ivf.php:7553-7557` | ❌ |
| S-1.2 | 同一舊案第二次附表五 | 未跑。靜態：3.0 6+4、不 keep | ❌ |
| S-1.3 | 三格加總 15 萬 | 未跑。靜態：`7558-7561` | ❌ |
| S-2.1 | 附表五 6／5／4 | 未跑。靜態：同 S-1.3 + `2721-2726` | ❌ |
| S-2.2 | 存檔後重整形成仍 30000 | 未跑。靜態 happy path 可還原 | ❌ |
| S-2.3 | 三格 7／1／5 萬 | 未跑。抽驗列：`7568-7571` | ❌ |
...
|---|---|---|---|---|---|---|
| S-1.1 | 補助行政 | 9／1 新案算出 10+0+5 | 開附表五、選方案 A | 申請日在主檔 | 未實跑 | 靜態數字對 |
| S-2.2 | 補助行政 | 手改形成後再打開還在 | 改三格、存、重整 | 無 | 改方案／次數會先重算蓋手改（F-9） | 存再開走 derive，未實跑 |
...
- F-3 🟢 PLUS 三格可改；3.0 兩格 readonly＋後端覆寫
...
- F-8 🟡 `shouldKeepPostedSubsidy3Amounts`（`ivf.php:7520-7533`）不理 `subsidy_amount_manual`。T-2 寫要看旗標；實作是 PLUS＋三欄有值就 keep（對 S-2.2 更寬）。建議對齊 tasks 或改成只看 manual
...
- F-13 🟡 形成＝limit−ovum−implant，髒資料可能負
...
- R-2 三格 — **符合（靜態）**。PLUS 解 readonly＋形成格。3.0 兩格＋後端覆寫。形成不落庫。手改存檔 keep POST。T-2 旗標字面有偏差（F-8），行為仍「手改不蓋」。
...
| 案例 | 取卵 | 形成 | 植入 | 上限 |
|---|---|---|---|---|
| S-1.1 PLUS 35 首次 | 100000 | 0 | 50000 | 150000 |
| S-1.2 3.0 35 第2 | 60000 | （隱藏） | 40000 | 100000 |
| S-1.3／S-2.1 PLUS 35 第2 | 60000 | 50000 | 40000 | 150000 |
| S-2.3 PLUS 40 首次 | 70000 | 10000 | 50000 | 130000 |
...
- `views/backend/ivf_subsidy_edit.php`：三格、PLUS JS、年齡鎖、OPU AJAX
...
S-1.1（9／1 三格 10+0+5）、S-1.2（8／31 仍 3.0 兩格）、S-1.3（第2次 6+5+4）、S-2.2（手改形成存再開）、S-4.1（fbt_date 列出 OPU）。
