> **正本聲明**:本檔是 Agent Memory 自動改進迴圈(autoloop)的**唯一契約正本**。
> 排程器注入的指示只指向本檔、不再複製全文;沒有第二份拷貝,就沒有內容漂移。
> 漂移缺口已在 2026-08-21 消除,見附錄 A「已知缺口」。剩下的 lockstep 只有
> 「本檔不能離開 `main`」——搬移或改名才要同步改 automation 的指示文字。
> 這份契約自己(§6)的原則仍是「寫得漂亮但不準,下一輪就從錯的前提開始」。
...
> **本檔相對原文有八處修正**,原因是執行環境從 Claude 排程任務遷移到 Cursor Automation
> (沒有 Google Drive 寫入能力),以及七處原文留白由 owner 裁決補上。
> 每一處都在附錄 A 逐條記帳,對應的節內也標了 `【修正 N】`。**不要憑印象假設某一節是原文** ——
> 想知道哪裡被改過,看附錄 A,不要看語氣。

> **agent 不得修改本檔**(§0 明訂「改這份 routine 自己 → 一律拒絕」)。
> 契約變更由 owner 改本檔;automation 指示只指向本檔,改契約不必同步改
> automation 設定。搬移或改名才要改那個指標(見附錄 A「已知缺口」)。
...
**【修正 8】不管成功或失敗,都要把這三個數字記下來填進 §6 的 `Drive 讀取` 欄位**:
HTTP 狀態碼、解析到的檔名數、最新的 GPT 檔名。這是本輪唯一一個**跨系統邊界**的
讀取動作,也是唯一一個 owner 無法從 git 事後複驗的環節 —— git 裡的東西隨時查得到,
Drive 那一刻讀到什麼只有當下的 agent 知道。不記就永遠只能靠推論。

⚠️ **【修正 6】Drive 讀不到 ≠ 沒有新輸入 —— 這是環境失敗,不是狀態判定。**

以下任何一種都算讀不到:`curl` 非零退出 / HTTP 非 200 / 回傳內容解析出的
**檔名數為 0**(資料夾實際上從來不是空的,所以 0 視為失敗是 fail-closed 的正確偏誤)。

發生時**必須**:

1. **不得**下「沒有新輸入」的結論,**不得**因此跳過本輪。
2. §2 健康檢查照做 —— 它完全不依賴 Drive。分支不健康就照 §2 修好並推。
3. 若因此寫了 §6 接續紀錄,`回應的 GPT 檔名:` 那一行**原封不動沿用上一份的值**。
   狀態不得前進 —— 沒讀到 Drive 就不知道該前進到哪裡,亂寫會讓下一輪從錯的前提開始。
4. 本輪類型寫「環境失敗:Drive 不可達」,並貼出**實際的** HTTP 狀態碼與解析到的
   檔名數。不要寫「Drive 好像有問題」,要寫 `http=403, names=0`。

**為什麼要特別寫這條**:讀不到 Drive 而靜默跳過,長得跟「真的沒有新輸入」一模一樣 ——
兩者都是「什麼都沒做就結束」。差別只有在紀錄裡寫出來才看得見。這個迴圈每小時跑一次、
沒人盯著,一個看起來正常的失敗可以持續好幾天不被發現,而那正是這個記憶系統自己在修的
那個 bug 形狀(狀態宣稱比實際證據強)。
...
- 本輪類型: 回應 GPT 建議 / branch 健康維護 / 環境失敗:<哪一項>
- 新增 commit: <SHA + 一行說明>
- **【修正 7】本檔 raw URL**: <完整網址,自己組,見 §7 第 7 步。這是必填欄位>
- **【修正 7】PR comment**: <「已貼:<comment 網址>」或「未貼:<實際原因>」。
  兩者擇一,不得省略,不得寫成指向別的欄位>
- **【修正 8】Drive 讀取**: <`http=<狀態碼>, names=<解析到的檔名數>,
  newest=<最新的 GPT 檔名>`。成功也要填,不是只有失敗才填。數字要是**實際跑出來的**,
  不是照抄本契約的範例>
...
7. **【修正 4】發布給 GPT 讀 —— git 是唯一正本,本端不寫 Drive。**
   ...
   **【修正 7】這條 raw URL 是無條件必填的。** 它只是一個字串,不需要任何工具就組得出來,
   所以「沒工具」不是不填的理由。填進第 6 節的 `本檔 raw URL` 欄位。

   然後把第 6 節內容貼成一則 PR comment。**永遠不要 merge。**

   **【修正 7】已知障礙 —— 先讀完再動手**:排程 harness 提供的 GitHub 工具通常綁在
   **它指派給本輪的那支自動產生分支**上,而 §3 要你忽略那支分支、去別的分支工作。
   兩者的 PR 對不起來,工具會失敗(實測:`Open Pull Request` 報「This branch is not
   pushed to the remote」;PR comment 工具沒有目標)。這**不是**你做錯,是設計上的錯位。

   依序試,第一個成功的就停:

   1. shell 有可用且已認證的 `gh` → `gh pr comment <PR 編號> --body-file <第 6 節那份檔>`
   2. harness 的 PR comment 工具能指定 §3 推導出的那支分支的 PR → 用它
   3. 該分支沒有 open PR → 開一個 **draft** PR(同樣受上述錯位影響,失敗就往下)
   4. 都不行 → **寫「未貼:<實際的錯誤訊息或原因>」**

   **【修正 7】做不到要說做不到。** `PR comment` 欄位**不得**寫成「見上方 PR 欄位」
   或任何指向別處的說法 —— 那讓「貼了」與「沒貼」在紀錄上長得一樣,而這份紀錄是
   下一輪與 owner 的唯一依據。沒貼不是失敗,**把沒貼寫成像貼了才是**。

   PR comment 貼不成不阻斷本輪 —— git 是唯一正本,GPT 讀得到 raw URL 就夠了。
...
## 修正 6 — §1 步驟二:Drive 不可達必須 fail-loud(2026-08-21)

**原文空白**:§1 只寫了怎麼讀 Drive,沒寫讀不到怎麼辦。

**為什麼這個空白危險**:讀不到而靜默跳過,與「真的沒有新輸入」在外部觀察上
**完全同形** —— 都是什麼都沒做就結束。迴圈每小時跑、沒人盯,這種失敗可以持續
好幾天不被發現。而修正 1 當初就是為了消滅「每小時空燒」才把狀態搬進 git;
這個空白等於在別的位置把同一個失敗模式放回來。

**裁決**:非 200、curl 失敗、或解析出 0 個檔名,一律視為環境失敗。不得推導出
「沒有新輸入」;§2 照做(不依賴 Drive);接續紀錄的「回應的 GPT 檔名」原封不動
沿用,狀態不得前進;本輪類型標「環境失敗:Drive 不可達」並貼實際 HTTP 狀態碼
與檔名數。

**已實測可達(2026-08-21,從 owner 本機)**:資料夾列表 `http=200`、解析出 17 個檔名;
`uc?export=download` 下載 13,889 bytes 全文。檔案 13KB 遠小於 Google 會插入
病毒掃描中介頁的門檻,直接回內容。**但雲端 agent 的出口與本機不同,首輪跑起來才算
真的證明** —— 這條修正存在的目的,就是讓首輪若失敗會大聲說出來而不是靜默跳過。

## 修正 7 — §6/§7:raw URL 與 PR comment 改成必填欄位(2026-08-21)

**觸發事件**:遷移後的**首輪實跑**(`20260821-1537-claude.md`)。該輪程式碼與驗證
都是好的(114/114 守衛、55 項不變量、eval PASS,owner 獨立複跑對得上),但 §7 第 7 步
兩件事都沒做到:接續紀錄裡**沒有** raw URL,PR #8 上**沒有**新 comment。
而收尾清單寫的是「raw URL + PR comment → 見本檔開頭的 branch 與 PR 欄位」——
開頭只有 `PR: #8`,既無網址也無 comment。

**根因(兩層,都不是 agent 偷懶)**:

1. **能力錯位**:harness 的 GitHub 工具綁在它指派給本輪的自動產生分支上,而 §3 要
   agent 忽略那支分支。工具因此失去目標 —— run summary 實際顯示
   `Failed to open Pull Request: This branch is not pushed to the remote`。
2. **契約留白**:§7 原文寫「有 GitHub 工具時…沒有 GitHub 工具時 branch 推上去就夠了」,
   **沒有規定做不到時要在紀錄裡留痕**。於是「做不到」與「做了」在紀錄上長得一樣。
   raw URL 那條更冤 —— 它根本不需要工具,只是一個字串,卻跟需要工具的 PR comment
   綁在同一段,被一起跳過了。

**裁決**:
- §6 模板新增兩個**必填欄位**:`本檔 raw URL`(無條件,不需工具)、
  `PR comment`(「已貼:<網址>」或「未貼:<原因>」二擇一)。
- §7 第 7 步寫明能力錯位這個已知障礙與四步降級順序(`gh` → harness 工具 →
  draft PR → 寫「未貼」),並禁止用「見上方欄位」這類指向搪塞。
- PR comment 貼不成**不阻斷本輪** —— git 是唯一正本。

**為什麼要動契約而不是只提醒一次**:這正是本迴圈反覆在修的那個形狀 ——
**宣稱比證據強**。首輪那份紀錄其餘部分誠實到會主動寫「我上一輪把它記成 FIXED,
是宣稱比證據強一級」,唯獨這一格滑掉了,因為契約沒要求它留痕。要求留痕之後,
沒貼就是紀錄上明明白白的一行,不需要有人去比對 PR 才發現。

## 修正 8 — §1/§6:Drive 讀取成功也要留痕(2026-08-21)

**原文空白**:修正 6 規定了「讀不到要 fail-loud、貼出實際狀態碼」,但**沒規定讀到了
也要貼**。於是成功的那條路徑不留任何痕跡。

**後果(實測)**:首輪 `20260821-1537-claude.md` 整份 348 行,關於 Drive 只有兩行檔名
(`回應的 GPT 檔名: 20260821-0829-GPT.markdown`),沒有狀態碼、沒有檔名數。owner 想
確認「雲端出口到底讀不讀得到 Drive」時,只能靠**間接推論** —— 那一輪修的
`GPT-P1-CURRENT-FACT-SCOPE` 只存在於 Drive 那份檔裡、git 裡沒有,所以它一定讀到了。
推論是對的,但每一輪都要重推一次,而且只在「有新 finding 被處理」時才成立;
一輪跳過的回合就完全無從判斷。

**裁決**:§6 新增必填欄位 `Drive 讀取`,格式
`http=<狀態碼>, names=<檔名數>, newest=<最新 GPT 檔名>`,**成功也要填**。

**為什麼值得一個必填欄位**:這是本輪唯一一個**跨系統邊界**的讀取,也是唯一一個
owner 事後無法從 git 複驗的環節 —— git 裡的東西隨時查得到,Drive 在那一刻回了什麼
只有當下的 agent 知道。把它記下來,「跳過」這個結論才有證據支撐;不記的話,
「真的沒有新輸入」與「Drive 回了空但沒觸發 fail-loud」在紀錄上仍然同形,
而那正是修正 6 想消滅的東西 —— 修正 6 只堵了明確失敗,沒堵住「看起來成功但其實沒讀到」。
