...
| `agents/` | `devflow-reviewer`(role=reviewer,唯讀收驗)、`devflow-adviser`(role=adviser,唯讀連敗診斷)——兩者 frontmatter 皆 `tools: Read`,不給 Bash/Edit/Write;plugin 載入後型別字串帶命名空間:`dev-flow:devflow-reviewer` 已在單次臨時載入下實測叫得出來,`dev-flow:devflow-adviser` 沒有被叫過、是照同一條命名規則推的(證據與三條邊界見 `agents/devflow-reviewer.md` 的「型別字串」節;本表由 `scripts/check-hooks-accounting.sh` 對帳) |
