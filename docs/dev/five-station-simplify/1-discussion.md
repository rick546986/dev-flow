...
## Problem
痛:母版七站加上例行 G1／Stage 3 `ACCEPTED`／G2,讓「方向、互動、契約」三次都要等人。現場 dogfood 與採用回報裡,這三次常被 owner 用 chat 一句「Treat as PASS／都過／可以」蓋過——等人的次數等於站數,審查物質卻沒進人腦。摺站若只殺等待、卻讓 Spec／Build 以「已經五站了」拿掉 ID 鏈、反模糊、T 四欄、acceptance seam,勾選會綠、工作沒做完。
...
- 禁則 X2 刪 token、X4 in-flight 套新機、X7 Agent 書面審冒充判定:notes/design/five-station-simplify-f0-state-machine.md:L189-L201
...
- `[Assumption]` 採用現場仍用同一套 chat 蓋章過 G1／G2:無採用逐字稿;風險=高(若為假,「殺例行停」的現場理由變弱,Must-keep 仍在);期限=F1 annex 前抽一案,過期不得把「現場都蓋章」寫進 4-spec 當已核事實。
...
- 採用現場兩案回報被叫做 G1／G2／G3:notes/dispatch-accounting-symmetry.md:L10、L361;STATUS 複述「現場真踩到」:docs/dev/STATUS.md:L49。
- 觀測實驗延後、禁先改 1–4 模板:notes/dispatch-parallel-feature-gaps.md:L279-L285。
- `[Assumption]` 兩條見 Exceptions;皆無採用逐字稿／未來寫手 log。

### Assumption 四欄
| 若為假影響什麼 | 影響級 | 怎麼驗 | 何時／由誰驗 |
|---|---|---|---|
| 「採用現場也 chat 蓋章」為假 → 殺例行停的現場理由變弱,但不改 Must-keep | 高 | 抽一採用案:G1／G2 twin 是否有人讀五格再寫 `verdict:` | F1 annex 前／owner |
...
| 採用現場回報 | 兩個專案踩到的洞被叫做 G1／G2／G3 | notes/dispatch-accounting-symmetry.md、docs/dev/STATUS.md | 是(本 tree) | 是 |
...
- G-out-3:後來寫規格／切任務的人,不能把「已經五站了」當成省略 ID 鏈、反模糊、T 四欄、或 RED→獨立審查 seam 的理由。省略後的勾選必須仍看得出是未完成。
...
- Must-keep 進後站 disposition:每條 M 在 Spec 有 R/S 或明示 Non-Goal;Build 的 T 四欄與 seam 對 M11／M1／M9／M10 不可選。
...
- [~] Q6:採用現場是否仍用 chat 蓋章過 G1／G2?(帶假設:是;風險=高;期限=F1 annex 前抽一案,過期不得當已核事實寫進 4-spec)
- [>] Q7:F1 牙怎麼釘「少一條 Must-keep=紅」、尤其 M11 假完成 T?→ 移交 Stage 2／F1
- [>] Q8:dual-read 2.1.0 欄位與舊檔不紅的缺省?→ 移交 F1 annex
- [>] Q9:Must-keep disposition 引用語法(不另發 ID 鏈)怎麼寫?→ 移交 Stage 2
- [>] Q10:coordinator event 與 rewrite cap 計數落點?→ 移交 F2
- [>] Q11:B1 命中與「chat 准開下一站」並存時,attestation 空是否仍機械拒?→ 移交 Stage 2(現行牙已拒;五站後要保持)
- [>] Q12:本 slug 自己 in-flight 之後走舊 7 還是等 F3?→ 移交整合／F3 cut 規則(brief §6:F3 前新開母版軌仍舊 7)
...
| 帶走什麼 | 若被 Spec／Build 偷走 | 人看見的假完成 | 本檔鎖在哪 |
|---|---|---|---|
| M1 ID 鏈;測試名含 S-id | T 不標 Covers;測試不叫 S | 勾選綠,Ship coverage 對不上 | G-out-3、AC-3 |
| M2 圍欄 | 實作者翻 1／2／3 補洞 | spec 空洞仍往下寫 | G-out-7 |
| M3 反模糊三律 | S 寫適當／TBD | 測試空轉也綠 | G-out-3、AC-3 |
| M4 Real-world→Demo→OC | 無 trigger 也省、有 trigger 也省 | UI 綠燈但人做不完 | G-out-6、AC-6 |
| M5 人寫 `ACCEPTED`／Ship `PASS` | Agent 代填 | 出貨 verdic 是機器的 | G-out-4、AC-4 |
...
| M9 Files ⊆ 5-tasks | 範圍外檔跟著合 | 任務板說沒改、diff 有改 | G-out-3、AC-3 |
| M10 驗證五律 | 派工者下場修、無原始輸出 | 審查看摘要當證據 | G-out-3 |
| M11 T seam | 跳 RED／跳獨立審 | **假完成 T**(C 線主風險) | G-out-3、AC-3 |
| M12 author≠approver | 自己簽自己 | 四眼只剩欄位 | G-out-4 |
| M13 html 重生 | 只丟 raw md | 人審不到頁 | 本 hop 仍產 html |
| M14 不可逆才 Quiz | 省 Quiz 或每次都 Quiz | 危險合進去,或第三人停點回來 | G-out-1 |
| M15 token／檔仍在 | 刪 G1／G2／`ACCEPTED` 好寫謂詞 | dual-read／舊 7 一次紅 | G-out-5、AC-5 |
| M16 F0 不改 graph／牙 | F0「順便」改路線 | 觀測被污染 | Non-Goals |
...
- AC-7(G-out-7):假設本檔 Constraints 表列了一條 Must-keep 或 Journey 痛點,當人讀到 Spec／Build,則該條有去向。
  ...
  - 看到什麼算對:每條高影響列都有處理／維持／Non-Goal／另開／仍待驗;沒有「Stage 1 寫過、後面消失」
...
- Q:採用現場的 G1／G2／G3 痛,跟「蓋章」是同一件事嗎?
  ...
  - 推理:採用兩案回報的是母版缺陷(散發路徑、守衛自打),名字剛好叫 G1／G2／G3,證明現場真的在走這些閘,痛是實的。那是「閘的物質有洞」。母版 dogfood 另證「閘的等待被蓋章」。兩件事都真:物質要留(且要有牙),等待不該例行。2026-08 連母版自己的完整觀測都還在排隊,更不能先改模板污染觀測——F0 禁改模板與這條一致。
  - 結論:CONFIRMED 現場痛支持留物質;dogfood 蓋章支持殺例行停。兩者不可互相取消。
...
- ⚠️ Q:若只殺等待、不釘 Must-keep 牙,最極端會怎樣?(發散)
  ...
  - 結論:OPEN 牙的形狀移交 F1;本 hop 只鎖定「少 M=違規」與「假完成 T 必須仍能被看見」。
- ⚠️ Q:本 hop 有沒有把 F0 當成已經施工?範圍有沒有長大?(盲點)
  - 事實:notes/design/five-station-simplify-brief-v3.md:L7-L9 docs/dev/STATUS.md:L50 本 hop brief(只 Stage 1、不改 STATUS、獨立)
  - 推理:F0 已落盤是 Context,不是本 PR 的實作範圍。隱含預設「後站會偷 Must-keep」無未來 log,已標 Assumption。另一隱含預設「owner 永遠是蓋章瓶頸」若為假,殺等待仍可能對(注意力有限),但要重估採用現場。本檔不改模板、不送 G1,避免跟 F0 禁令撞車。
  - 結論:CONFIRMED 本 PR 只落討論;不重開十條;不改 STATUS。Q6 過期不得當已核事實。
