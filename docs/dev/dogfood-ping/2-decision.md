...
> 把 `1-discussion.md` 的 lean 收成 Decision。本檔送 G1：`verdict` 空、`status` draft、OC-1～OC-4 待人審。契約維持 `2.0.0`。本 hop 不升 plugin、不碰 `#163`／host-stack-fit 正本、不做假 PreToolUse。
...
- Out：`#163` G3／PF-0；升契約／bump plugin；假 PreToolUse；生產業務邏輯；另開 scratch repo；只寫 docs 不交腳本；固化 #159 Stage 5 模板欄（Intent 白話除外，見 OC-2）。
...
|---|---|---|---|---|---|
| OC-1 | Verify **至少一條可自動跑**（腳本本身或最小測試）；是否掛進 `devflow-check`／selftest 本輪可後置到 4-spec。使用者只被問到 Q3 未定；「至少可自動跑」是 owner 對 `[~]` 的收窄 | dogfood 要可觀測；純手動無法當回歸牙 | `1-discussion.md` Q3 `[~]` 假設句；Goals 可觀測題 | 改成純手動；SC-1／回歸入口要重寫 | 待人審 |
| OC-2 | Stage 5 **Intent 白話本輪先寫**；#159 模板欄位固化另票。承接 Q4 `[>]` | 避免本 dogfood 變模板基建大包 | `1-discussion.md` Q4 建議句；Non-Goals 不順便升契約 | Scope 加上改 Stage 5 模板；與試跑纏在一起 | 待人審 |
| OC-3 | 腳本路徑固定 `scripts/dogfood-ping.sh`，並同步 file-map／相關地板。使用者 Goals 寫「建議」；路徑鎖定是 owner 延伸 | 路徑不定 → Stage 4／6 各自發明；地板必碰已是已知事實 | `1-discussion.md` Goals 首條「建議」；Context file-map 地板 | 改名／改目錄；SC-1／file-map 對照跟著變 | 待人審 |
| OC-4 | 本 dogfood **不**把產出塞進進行中的 `docs/dev/host-stack-fit/`，Example 落點獨立 `example/dogfood-ping`。使用者已選 Ex；「不混 host-stack-fit 樹」是收窄 | 避免與 `#163` 未結 G3 攪線 | `1-discussion.md` Non-Goals「不解決 #163」；Q1 Example 路徑 | 改塞進既有 feat 樹；審查與 merge 邊界糊掉 | 待人審 |
...
- 本 preview PR 只服務 Stage 1–2 手機預覽；實作腳本留待 G1 後。
...
- G1 | — | 待人審；`verdict` 空、`status` draft；OC-1～OC-4 待裁決
