...
# feedback record(W2 runtime `feedback` 子命令寫入;kind 仍是 jev,`record_type=feedback` 區分)
FEEDBACK_ALLOWED_KEYS = {
    "record_type", "evaluation_id", "case_id", "gate", "verdict", "source", "reviewer_ref", "session_ref",
    "feedback_at", "artifact_hash", "evidence_hash", "head_sha", "suspect", "occurred_at",
}
FEEDBACK_VERDICTS = ("agree", "overturn")
...
def assert_durable_safe(record):
    ...
    jev = record.get("jev", {})
    is_feedback = jev.get("record_type") == "feedback"
    extra = set(jev) - (FEEDBACK_ALLOWED_KEYS if is_feedback else JEV_ALLOWED_KEYS)
    if extra:
        problems.append("jev{} 出現白名單外的鍵 %s" % sorted(extra))
    if is_feedback:
        if jev.get("verdict") not in FEEDBACK_VERDICTS:
            problems.append("feedback.verdict 只准 %s(none != agree)" % list(FEEDBACK_VERDICTS))
        if not isinstance(jev.get("suspect"), list) or any(not isinstance(x, str) for x in jev["suspect"]):
            problems.append("feedback.suspect 必須是字串清單(空 = 不可疑)")
...
def build_feedback_record(evaluation, feedback, suspect):
    """人／fresh agent 對某 evaluation 的 label(agree/overturn)。durable 只放 ID/hash/來源/可疑理由。
    suspect 非空的 feedback 一樣落盤(留痕),但 report 不算進 n(provenance.feedback_suspect 是唯一判準)。"""
    ev = evaluation
    verdict = feedback.get("verdict")
    if verdict not in FEEDBACK_VERDICTS:
        raise JevError("feedback.verdict 只准 %s" % list(FEEDBACK_VERDICTS))
    for key in ("source", "reviewer_ref", "session_ref", "feedback_at"):
        if not isinstance(feedback.get(key), str) or not feedback[key].strip():
            raise JevError("feedback 缺 %s(fail-closed:缺 provenance 不落盤)" % key)
    suspect = sorted(str(x) for x in (suspect or []))
    fields = {
        "gate": ev["gate"], "eval": ev["evaluation_id"], "case": ev["case_id"], "verdict": verdict,
        "source": feedback["source"], "reviewer": feedback["reviewer_ref"][:MAX_KEY_TEXT],
        "artifact": (feedback.get("artifact_hash") or "sha256:-")[7:23],
        "evidence": (feedback.get("evidence_hash") or "sha256:-")[7:23],
        "head": (feedback.get("head_sha") or "-")[:12],
        "suspect": (",".join(suspect) or "-")[:120],
    }
    record = {
        "event_id": new_ulid("evt"),
        "kind": "jev",
        "title": "%s feedback %s %s verdict=%s source=%s%s" % (
            TITLE_PREFIX, ev["gate"], ev.get("slug") or "-", verdict, feedback["source"],
            " suspect" if suspect else ""),
        "body": _kv(fields),
        "occurred_at": feedback["feedback_at"],
        "session_id": "jev-feedback-%s" % ev["evaluation_id"],        # 每 writer 自己的檔
        "signal": "high",
        "paths": [],
        "source_type": "jev",
        "source_ref": "%s:%s:%s:feedback" % (ev["gate"], ev.get("slug") or "-", ev["evaluation_id"]),
        "jev": {
            "record_type": "feedback", "evaluation_id": ev["evaluation_id"], "case_id": ev["case_id"],
            "gate": ev["gate"], "verdict": verdict, "source": feedback["source"],
            "reviewer_ref": feedback["reviewer_ref"], "session_ref": feedback["session_ref"],
            "feedback_at": feedback["feedback_at"], "artifact_hash": feedback.get("artifact_hash"),
            "evidence_hash": feedback.get("evidence_hash"), "head_sha": feedback.get("head_sha"),
            "suspect": suspect, "occurred_at": feedback["feedback_at"],
        },
    }
    assert_durable_safe(record)
    return record
