...
| 風險 | 對策 |
|---|---|
| 填檔牙形狀未釘，Stage 4 之前各寫各的 | Decision 只鎖「有重綁 SHA 或明示 FAIL，否則紅」。Stage 3 已用 throwaway fixture 跑過擬定形狀（void-only 紅／rebind 或 FAIL 綠）；欄位／腳本名進 4-spec；OC-1 沿用既有檢查家族。見 3-prototype Result |
| 咬填檔時誤殺 `N_A_NO_INCOMING` 或未跑 2c 的 draft | 牙只在宣稱勾過整合項／送 G3／明示 FAIL 時發動；n-a 與未結論 draft 不預先紅。Stage 3 fixture `filled-n-a.md`／`filled-draft.md` 皆判 N/A。細節 4-spec |
...
- Q6 錨句候選（4-spec 再釘確切字）：「整合回歸在 Final Fresh 之前」「ALREADY_SYNCED 不得只寫證據不算數」。Stage 3 補擬定欄位：Rebound Source SHA（≥7 hex）或「本項 FAIL」。
- Stage 3 已行使（命中 7 條；CLI Demo；無跳過 OC）。Human verdict 仍 NOT_REVIEWED，3-prototype status 留 draft。
- 本 hop 產審頁用 `scripts/build-stage3-html.py --action`，不手包 html-shell，不把審頁塞進 `build-gate-twin.py` STAGES。
...
- prototype 回寫 | 2026-09-12 | Stage 3 CLI Demo：AS-1 填檔牙擬定形狀（void-only 紅；重綁 SHA／明示 FAIL 綠；n-a 與未結論 draft 不預先紅）。活教師五條路徑仍舊、intended 改口已展示未改正本。順序維持 tip 2c 整合 → 2d Fresh。不假裝 G2／G3。見 `3-prototype.md` Result。
