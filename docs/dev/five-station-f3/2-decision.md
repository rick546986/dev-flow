...
# 2. 收斂 — 五站 F3（Writer B：cut 證明／讀鍵縫／graph+dual-read／doctor 誠實／成功≠空切）

> 把 `1-discussion.md`（C+soft-fix）收成一個選定方案。Lane = **full**。本 hop 只落 `2-decision.md` + `2-decision.html`。**不改 STATUS／HISTORY**。頂欄 `status: draft`、`verdict` 空。**不發明 G1 PASS**、不合併、不寫碼、不 bump 契約。
> Stage 1 頂欄仍 `status: draft`、當時寫「不送 G1」。Owner 2026-09-14 以「ok」開本站。1-discussion 留當時說法；改口記本檔。
> Writer B 主軸：**F3 = 新 slug 預設五站的 cut**。繼承 F2 完成樹 **與** 4A 三前置形狀。`2.1.0 ≠ cut`。禁 silent `True`。保護 in-flight／token。doctor 綠 ≠ 路條。成功必須可量測，檔在／用字／函式真 ≠ 完。不重開 F2 park D-1／D-2／D-3／F-c-4。不重開「要不要三條」。
> 本檔含 owner 自拍板（OC ledger）。G1 未審；OC 全「待人審」。
...
| 1C | **STATUS／guide 用語 = cut**（或函式 silent `True`） | 最快宣告 | 看板 ≠ 刀；用字 ≠ 行為；silent True = AC-4 反面；#359 已開 Active 仍 `f3_cut=False` | 低 | `1-discussion.md:L202` Q14；`:L205` Q15；`:L213-L214` Q23／Q24；`docs/dev/HISTORY.md:L721-L725` no F3 opened；`guides/guide-dev-flow.html:L573` 仍七站單行 |

### 決策點 2：key／read 縫（Q16／Q17／Q27）
| 方案 | 摘要 | 優 | 劣 | 成本 | 依據(`檔:行` 或 `[Assumption]`) |
|---|---|---|---|---|---|
| 2A | **同刀或先宣告 + 修讀鍵**。正本鍵 `devflow_contract_version` bump 到 2.1.0 時，`contract_version()` **必須先讀正本鍵**（可 fallback 舊誤寫 `version`／`contract_version`）。`supported_contract_versions` **同刀**加 `2.1.0`。hops 預設五站 **不得早於** 2.1.0 已宣告（同刀或先宣告）。**2.1.0 仍 ≠ cut**；缺 cut 位元 → `allow_legacy()` | 擋「bump 了所以已宣告」；擋 SLOT-REJECT；繼承 4A | 本 hop 不 bump；後站要動 reader／契約／supported | 中 | `1-discussion.md:L204` Q27 空字串已核；`:L206-L207` Q16／Q17；`scripts/five_station_f2.py:L260-L268` 不讀正本鍵；`devflow-contract.json:L1-L2` 正本鍵=`devflow_contract_version`=`2.0.0`；`notes/design/five-station-simplify-f1-dual-read-annex.md:L22-L24` SLOT-REJECT。fallback `[Assumption]`（4-spec 可收窄只讀正本） |
...
| 方案 | 摘要 | 優 | 劣 | 成本 | 依據(`檔:行` 或 `[Assumption]`) |
|---|---|---|---|---|---|
| 6A | **三把鎖 + 繼承鎖寫進 Non-Goals、後站不准改成可選**：(1) 不刪 G1／G2／`ACCEPTED` token 與檔；(2) 不折 in-flight（含本目錄、F2、simplify、任何已有 1–7 `.md`）；(3) 不拿本 slug 當活五站白老鼠。另鎖：不 silent True；2.1.0 ≠ cut；doctor／marketplace／cache ≠ ticket；不炸模板全文；不重開 F0 十條／三 cap／F2 park／4A 形狀；本 PR 只 Decision＋html | 對齊 brief §7 不做列；本目錄當 freeze 樣本 | 採用端預設痛在 cut 落地前仍在（本來就該如此） | 低 | `brief-v3.md:L180-L182`；`1-discussion.md:L175-L186` Non-Goals 初稿；`:L196-L201` Q7／Q12／Q13 |
...
## Decision
採 **1A+2A+3C+4A+5A+6A**：F3 做 **Cut**——cut 當下沒有 1–7 `.md` 的新 slug 預設五站；舊 7 只服務 freeze + dual-read；guide／STATUS 用語切五站。**cut attestation SoT＝人類可見獨立紀錄**（誰／何時／讀哪個條件）。`f3_cut_happened()` 是讀端不是 SoT；silent `return True` 不算切。2.1.0 宣告 ≠ cut；三前置形狀繼承 F2 4A／S-7.1（宣告 2.1.0 ∧ ¬in-flight ∧ cut），缺一 → `allow_legacy()`。讀鍵縫必須修：`contract_version()` 先讀正本鍵 `devflow_contract_version`；只 bump 正本、不修讀端 → `declared` 仍假。hops 預設五站不得早於 2.1.0 已宣告。graph **與** dual-read **都要**：未宣告採用端仍舊 7；cut 後新 slug 不再例行進 `N7-g1`／`N6-g2`；這兩節點 **不刪**。doctor 不改握手語意，只加 `supported` 清單；漏加則誠實 INCOMPATIBLE；綠 ≠ ticket。F3 完＝同一電池 (a)(b)(c) 都能獨立紅、也能一起綠。不選 1B／1C／2B／2C／3A／3B／3D／4B／4C／5B／5C／6B／6C。

## Decision 約束（後站不准改成可選）
1. **silent `True` ≠ cut。** 1C／5B 已拒。後站若把「`f3_cut_happened` 回真」寫成完成條件 → 回本站。
2. **2.1.0 ≠ cut。** 第三位元可獨立為假。把 bump 寫成 cut＝翻 Q26／4A。
3. **可見紀錄為 SoT，函式只讀。** 語意槽（誰／何時／讀哪個條件）人指得到。鍵名／檔名 OPEN。
...
5. **讀鍵縫。** 本 tree `contract_version()` 讀 `version`／`contract_version`、不讀 `devflow_contract_version`、回 `""`＝已核。F3 落地必須讓正本鍵 2.1.0 → `declared` 真。只 bump 正本不算已宣告。
...
15. **三把 Non-Goals 鎖**：不刪 token；不折 in-flight；不白老鼠。把其中一把標可選＝翻 Decision。
...
| PRE-210-NE-CUT | PRE | 2.1.0 已宣告、cut 位元假 → `allow_legacy()`；理由含 cut 未發生（綠格：合法拒） |
...
減任一列 = 翻本 Decision。4-spec 可加列，不可把「函式真／檔在／F2 綠／用字」加成通過條件。

## Rejected Alternatives
| 不選 | 一句棄因 |
|---|---|
| 1B 2.1.0 本身 = cut | 違 4A／Q26；第三位元無法獨立為假。 |
| 1C 用語或 silent True = cut | 看板 ≠ 刀；函式真無紀錄 = AC-4。 |
...
第二個假綠是讀鍵縫。正本鍵是 `devflow_contract_version`；F2 reader 讀另外兩個鍵，本 tree 回空字串＝已核。只 bump 正本會讓人以為「已宣告 2.1.0」，live 仍 legacy。2A 逼同刀修讀端，並鎖 hops 預設不得早於宣告——否則不是 cut，是 SLOT-REJECT 或遠端改線。2.1.0 仍不是 cut。
...
成功定義沿 F2 反 hollow：同一電池三路。函式真、檔在、只 F2 綠、只改用字，任一被標 F3 綠 → 必須紅。
...
- **SC-BATTERY**(G-success-1)：存在**單一入口**跑完整電池。exit 0 **當且僅當** NEW5 組、OLD7 組、TOKEN 組都過。缺一組、跳過一組、或入口只轉呼叫 `scripts/test-five-station-f2.sh` → 非 0。觀測：該入口的原始 stdout／exit。
...
- **SC-PRE-210-NE-CUT**：PRE-210-NE-CUT 綠。2.1.0 已宣告、cut 假 → `allow_legacy()`；理由含 cut 未發生，不是「已宣告所以切了」。
...
## Scope & Non-Goals(定稿)
- **In**：1A 可見紀錄為 cut SoT（函式只讀；用語／2.1.0／看板 ≠ SoT）；2A 修讀鍵 + 2.1.0 同刀或先於 hops 預設（仍 ≠ cut）；3C graph 行為 + dual-read、節點不刪；4A doctor 只加清單、綠 ≠ ticket、誠實紅；5A 同一電池三路與具名 CASE（極性＝注入壞行為該格紅）；6A Non-Goals 鎖。Q15–Q27 全有去向。本 PR 只 Decision＋html。
- **Out（鎖死，後站不准改成 In）**：
  ...
  5. **2.1.0 當 cut**；重開 4A 三前置形狀。
  6. **doctor 綠／marketplace update／plugin cache 當 cut 或路條**。
  ...
  9. **刪 `N7-g1`／`N6-g2` 節點**；一次大爆炸改模板全文。
  10. 放寬 hop≤2／Decide≤1／Goal reopen≤1；重開 F0 十條；重開 F2 park D-1／D-2／D-3／F-c-4。
  11. 把「檔在」或「F2 綠」當 F3 完成。
  12. 本 PR 實作 cut／改 `graph.yaml`／bump 契約／改 doctor／改 coordinator、填 G1 PASS、合併。
  13. 選定 attestation／倉的 JSON 鍵名或檔名（4-spec）。
  14. 改已經 freeze 的 slug 的路線。
...
| OC-3 | **讀端先讀正本鍵 `devflow_contract_version`**（可 fallback 舊誤寫鍵）。使用者只帶 Q17「仍假」；「先讀正本」是延伸 | 不修則 bump 正本仍 `""`；2B 雙寫會把 bug 當契約 | `1-discussion.md:L204` Q27；`:L207` Q17；`five_station_f2.py:L260-L268`。延伸 `[Assumption]` | READ-SEAM 無法紅；假宣告 | 待人審 |
...
| OC-8 | **把 AC-9 收成具名 CASE 表＋同一入口**（上表 20 列，只准加不准減）。使用者要 measurable SC；CASE 名與「預期紅」格子是延伸。極性：注入壞行為 → 該格紅 | 不具名則後站可把紅格改可選或拆兩支腳本 | `1-discussion.md:L269-L272` AC-9。延伸 `[Assumption]` | CASE 消失；SC-HOLLOW 對不到 | 待人審 |
...
| OC-10 | 本 Decision hop **不**跑 `status-update.sh`、不改 HISTORY、不改 1-discussion 頂欄、**不發明 G1 PASS**、不合併、不寫 cut 碼。標**流程層** | 母版 STATUS 只在整合分支維護；使用者：draft、No G1 invent、ONLY 2-decision+html | `docs/dev/STATUS.md:L10-L26`；本 hop brief | PR 帶 STATUS 或自填 PASS，與並行 session 互蓋 | 待人審 |
...
- 原文獨立收斂（Writer B）；金本＝C+soft-fix `1-discussion.md`。不讀未存在的 A 線 Stage 2。
- attestation 檔路徑／鍵名交 4-spec，本檔只鎖「人指得到、函式只讀」。
...
- 本 hop 不送 G1 | 2026-09-14 | Decision hop：`verdict` 空；OC 全「待人審」；不跑 N8 三連動。不發明 PASS。
