...
# 1. 討論 — 五站 F3（Implementer C + soft-fix：cut 證明／前置／graph／doctor／成功≠空切）
...
> Soft-fix 吸收(L1):#359 已開 Active `five-station-f3` @ 1-discussion G1⬜(不是空板);HISTORY F2 G3 引 L721–L725;2.1.0 ≠ cut、第三位元可獨立為假(F2 4A 繼承 `[x]`);live reader 錯鍵回空字串=已核事實;F2 D-1 檔案地圖列 ≠ cut;可選本 hop F2 電池綠當 Evidence;`[>]` 標五家族;不重開 F2 park D-1／D-2／D-3／F-c-4;graph 四選項未選。不選 attestation 格／不改 graph／不 bump 2.1.0。Interview Log 仍 8。
...
- brief §6 四條連讀(freeze／新 slug／舊機械／採用端):notes/design/five-station-simplify-brief-v3.md:L165-L168
...
- F2 coordinator G3 PASS;HISTORY 明寫 park D-1／D-2／D-3／F-c-4 與 **no F3 opened**(刀未切,不是看板沒開):docs/dev/HISTORY.md:L721-L725
- #359 已開 Active `five-station-f3` @ 1-discussion／full／rick／G1⬜ G2⬜ G3⬜;當時無 `1-discussion.md`;不發明 G1:docs/dev/HISTORY.md:L725-L729 docs/dev/STATUS.md:L32-L34
- F2 7-review 頂欄 `verdict: PASS`／`status: approved`;標題鎖 **F2 knife only**:docs/dev/five-station-f2/7-review.md:L1-L12
- F2 D-1:guide **檔案地圖列** ≠ F3 cut;`EXPECTED_MAPPED_FILES` 加列不是切預設:docs/dev/five-station-f2/7-review.md:L445
- F2 選定 4A:三前置全要=契約已宣告 2.1.0 ∧ 非 in-flight ∧ **F3 cut 已發生**;缺一 → `allow_legacy()`;2.1.0 ≠ cut;第三位元可獨立為假:docs/dev/five-station-f2/2-decision.md:L79
- F2 S-7.1:三前置缺一條就 `allow_legacy()`;不建五站機:docs/dev/five-station-f2/4-spec.md:L757-L761
- STATUS Backlog A **仍寫**「下一刀 F1 teeth＋dual-read annex」—— F1／F2 已出貨,此列是過期看板,不是「F3 已切」:docs/dev/STATUS.md:L52
...
- F2 `contract_version()` 讀 `version` 或 `contract_version`,**不讀** `devflow_contract_version`;本 tree 該函式回傳空字串(已核事實,不是假設):scripts/five_station_f2.py:L260-L268
...
- 本 hop 已跑 `scripts/test-five-station-f2.sh`(2026-09-14,`origin/main` `539679f`):exit 0;`failed=0`(回歸地板,≠ F3 完)
...
- Backlog B 凍 Stage 1–4 模板直到完整 full-lane 觀測做完:docs/dev/STATUS.md:L50
...
- STATUS／HISTORY 當刀口 log:人記得改看板;#359 已開 F3 Active,Backlog A 仍停在「下一刀 F1」。看板列 ≠ cut。
...
- 本 tree 已核:上列 Context 出處(2026-09-14 讀過 `origin/main` `539679f`,行段支持斷言)。
- F1／F2 已出貨:docs/dev/HISTORY.md:L659-L663、L721-L725。Active 已開 F3 列:docs/dev/STATUS.md:L32-L34 docs/dev/HISTORY.md:L725-L729。Backlog A 過期:docs/dev/STATUS.md:L52。
- coordinator／doctor／契約／graph／token:上列 scripts／hooks／skills／guides 出處。
- F2 D-1 檔案地圖 ≠ cut:docs/dev/five-station-f2/7-review.md:L445。F2 4A 三前置:docs/dev/five-station-f2/2-decision.md:L79。
- 電池:本 hop 已跑 `scripts/test-five-station-f2.sh`,exit 0、`failed=0`(地板,≠ F3 完)。
...
| F2 完成樹 | coordinator 預設舊 7、G3 PASS、4A、D-1、無 F3 刀 | docs/dev/five-station-f2/2-decision.md、4-spec.md、7-review.md、scripts/five_station_f2.py、docs/dev/HISTORY.md | 是(使用者點名 prior knives G3 PASS) | 是 |
...
| 看板 | Active=#359 已開 F3 @ 1-discussion G1⬜;Backlog A stale;刀未切 | docs/dev/STATUS.md、docs/dev/HISTORY.md | 是(本 tree) | 是 |
| F2 電池 | 本 hop 回歸地板仍綠;≠ F3 完 | scripts/test-five-station-f2.sh(本 hop 已跑) | 是(本 tree) | 是 |
...
- 三前置形狀繼承 F2 4A／S-7.1:**已是**宣告 2.1.0 ∧ 非 in-flight ∧ cut 已發生。缺一條 → `allow_legacy()`。2.1.0 ≠ cut。doctor 綠、marketplace update、plugin cache **不是**第四條。本討論不重開「要不要三條」。只移交第三位元**怎麼被指認**(Q20)。
- 本 hop 不選定:要不要 bump `devflow-contract.json`、要不要改 `graph.yaml`、要不要改 `_doctor_impl.py`、attestation 寫在函式／檔／契約／看板哪一格。
- graph 四選項(未定案,Q21):**改 `graph.yaml` 預設**／**宣告 2.1.0 dual-read**／**兩者都要**／**只翻 coordinator、留下 `N7-g1`／`N6-g2`**。不得默選。
...
- 本 hop 不送 G1;status 留 draft;不宣稱 Human Stage1 PASS;soft-fix 仍不改本 PR 內 STATUS;合併由 standing 在 CI 綠後做。
- **鎖:不刪 G1／G2／`ACCEPTED`;不把 in-flight 折成五站;不拿本 slug 當活五站白老鼠。**
- 不一次大爆炸改模板全文(brief §7 明文不是 F3)。
- 不重開 F0 十條;不放寬 hop≤2／Decide≤1／Goal≤1。
- 不重開 F2 park 的 D-1／D-2／D-3／F-c-4;不重開 F2 4A 三前置形狀。
- 不在本討論選定 attestation 落點、graph 四選項、或契約鍵怎麼對。
- 不把「doctor 綠」或「marketplace update」寫成 cut。
- 具名 hollow 不得當完:`f3_cut_happened==True`、檔在、只 F2 綠。guide 用字也不算。
- 不把「已經 cut 了」解讀成 Must-keep 也可以 hop 掉。
- 不發明本 slug 的 G1／G2／G3 PASS。

## Open Questions
- [x] Q1:lane 是否 full?→ 使用者:full
- [x] Q2:本 hop 是否只 Stage 1、不改 STATUS、不合併、不發明 G1?→ 原派工:是。status 留 draft。soft-fix 仍不改本 PR 內 STATUS;合併由 standing 在 CI 綠後做
...
- [x] Q5:prior knives 是否 G3 PASS、F3 刀是否已切、看板是否已開?→ F1／F2 HISTORY G3 PASS;F2 明寫 no F3 opened(刀);#359 已開 Active `five-station-f3` @ 1-discussion G1⬜(看板,不發明 G1)
...
- [x] Q14:STATUS Backlog A「下一刀 F1」算不算現況或已切證據?→ Active 已開 F3 列(#359);A 列 stale;不是 F3 已切
- [x] Q26:F2 4A 三前置形狀是否仍要重開?2.1.0 可否當 cut?→ 否。繼承 4A／S-7.1:2.1.0 ∧ ¬in-flight ∧ cut;2.1.0 ≠ cut;第三位元可獨立為假。本討論不重開形狀
- [x] Q27:live `contract_version()` 讀錯鍵、本 tree 是否回空字串?→ 是。已核事實:讀 `version`／`contract_version`,不讀 `devflow_contract_version` → `""`
...
- [~] Q19:F3 成功是否=同一電池 (a)cut 後新 slug 預設五站 (b)OLD7 freeze (c)token 在,缺一即紅?(帶假設:**是**;`f3_cut_happened==True`／檔在／只 F2 綠都不算;風險=高;期限=F3 Decision,過期擋 hollow)
- [>] Q20:**attestation** —— 第三位元怎麼被指認?改函式讀檔／契約 2.1.0 本身／人類 verdict／STATUS 用語／獨立標記?本討論不選。移交 F3 收斂
- [>] Q21:**graph-vs-dual** —— 四選項:改各站 `graph.yaml` 預設(拿掉 `N7-g1`／`N6-g2`)／宣告 2.1.0 dual-read／兩者都要／只翻 coordinator、留下 `N7-g1`?brief「做」列是 guide／STATUS 用語;F2 Out of Scope 把 graph 用語捆進 F3。本討論不選。移交 F3
- [>] Q22:**doctor** —— F3 要不要改 `hooks/_doctor_impl.py`,還是只動 `supported_contract_versions` 清單?移交 F3
- [>] Q23:**attestation** —— 「guide 用語切五站」算哪些檔?至少 `guides/guide-dev-flow.html` 仍寫七站單行。F2 D-1:檔案地圖列 ≠ cut。移交 F3
- [>] Q24:**attestation** —— brief 要 F3 切 STATUS 用語,但 feature branch 禁碰 STATUS 正本——用語切走 companion、還是本刀例外?移交 F3(與看板政策對帳)
- [>] Q25:**success** —— 第一隻活五站 slug 叫什麼、何時開?不是本目錄、不是已 freeze 的 F1／F2 母軌。移交 F3;本討論不發明名字
...
- **Owner-locked 三前置形狀:**F2 4A／S-7.1 已鎖「缺一條就舊 7」。2.1.0 ≠ cut。本討論不重開形狀。第三位元「讀哪／怎麼指認」仍 OPEN(Q20)。
- 詞條(語言,不是方案):**cut**=新 slug 預設從舊 7 改五站這一件事,不是刪閘、不是折舊 slug。**cut attestation**=人指得到的「已切」紀錄,不是函式回傳值本身。**三前置**=2.1.0 已宣告 ∧ ¬in-flight ∧ cut 已發生。**空切**=用字／函式／檔在變了,新 slug 行為沒變,或舊 slug 被折。**正本鍵**=`devflow_contract_version`。**讀鍵縫**=F2 `contract_version()` 讀的 `version`／`contract_version` 與正本鍵不是同一個。**graph 用語**=指南／看板把七站改稱五站。**graph 行為**=預設 hop 不再例行停 `N7-g1`／`N6-g2`。**in-flight freeze**=已有 1–7 `.md` 的 slug 走舊 7。**白老鼠**=拿本目錄或已 freeze 母軌當第一隻活五站。**hollow**=`f3_cut_happened==True`／檔在／只 F2 綠 冒充 F3 完。本 hop 不寫進長期記憶。

### F3 移交種子(不是施工)
- Q15–Q19 過期 → 擋本 slug G2(不得把假設升格成已核)。
- Q20–Q25 維持 `[>]`;後站不得假裝 Stage 1 已選落點。家族:Q20／Q23／Q24=**attestation**;Q21=**graph-vs-dual**(四選項);Q22=**doctor**;Q25=**success**。前置形狀已 `[x]`(Q26);只移交第三位元怎麼被指認。
- Q21 是 C 線主縫:brief 與 F2 Out of Scope 對 graph 的句子不一致,必須在 Decision 對帳四選項,不能默選。
- Q17 是前置主縫:只 bump 正本鍵可能切不動 `declared`;空字串已核(Q27),怎麼修仍 `[~]`。
- Q26／Q27 已解;後站不得把「2.1.0 就是 cut」或「三前置要不要三條」重開。
...
|-- STATUS Active          [five-station-f3 @ 1-discussion G1]
+-- Backlog A             [stale: still says next=F1]

cut-attest                [OPEN; Q15/Q20]
|-- silent True           [hollow]
|-- human-visible record  [who/when/which]
|-- contract 2.1.0 as cut [!= cut; Q26]
+-- STATUS wording        [board != knife]

preconditions             [4A inherited; Q26]
|-- 2.1.0 declared        [!= cut]
|-- not in-flight
|-- cut happened          [OPEN how; Q20]
|-- reader ""             [confirmed; Q27]
|-- 2.0.0 + five hops     [SLOT-REJECT]
+-- supported list        [doctor may go red]

graph                     [OPEN; Q21 four options]
|-- change graph.yaml
|-- declare 2.1.0 dual-read
|-- both
+-- coordinator only      [leave N7-g1]
...
|-- doctor green as cut
|-- f3_cut_happened==True -> hollow
|-- file exists           -> hollow
+-- only F2 green         -> hollow
```

## Interview Log(推理鏈外顯)
- Q:F3 這一刀到底做什麼、為什麼 F2 做完還不能當已經切?
  - 事實:notes/design/five-station-simplify-brief-v3.md:L166 notes/design/five-station-simplify-brief-v3.md:L180-L182 scripts/five_station_f2.py:L276-L278 docs/dev/HISTORY.md:L721-L725 docs/dev/STATUS.md:L32-L34
  - 推理:brief 把 coordinator 與 cut 拆成兩刀。F2 G3 明寫 no F3 opened(刀)。#359 只開看板 G1⬜,不是 cut。函式恒 False,live 一律 legacy。預設路線沒變,所以現場痛(新 slug 仍等人、chat 仍蓋章)還在。
  - 結論:CONFIRMED F3 = 新 slug 預設五站 + 舊 7 只服務 freeze/dual-read + 用語切;F2 完 ≠ 已切;Active 已開 ≠ 已切。
- ⚠️ Q:為什麼「把 `f3_cut_happened` 改 `True`」不能當 cut attestation?
  - 事實:scripts/five_station_f2.py:L276-L278 notes/design/five-station-simplify-brief-v3.md:L180 docs/dev/five-station-f2/4-spec.md:L826-L830 docs/dev/five-station-f2/7-review.md:L445
  - 推理:F2 把 cut 做成一個永遠假的布林,好測「未切」。若 F3 只翻真,人在 tree 裡看不到誰准了、讀哪個條件、graph／契約有沒有一起動。這跟 F2 D-1「檔案地圖列 ≠ cut」同一形:布林／加列是實作開關,不是出貨證明。可見紀錄的落點未選(Q20)。
  - 結論:NEEDS_VERIFICATION silent `True` 不合法(Q15 `[~]`);過期不得當已核寫進 Decision。
- ⚠️ Q:為什麼 2.1.0 宣告必須跟 hops 預設同動?讀錯鍵又為什麼是縫?
  - 事實:notes/design/five-station-simplify-f1-dual-read-annex.md:L22-L24 scripts/five_station_f2.py:L260-L268 scripts/five_station_f2.py:L286-L293 devflow-contract.json:L1-L2 docs/dev/five-station-f2/2-decision.md:L79
  - 推理:F2 4A 已鎖三前置 AND,2.1.0 ≠ cut。F1 已釘 2.0.0+五站 hops=紅。本 tree `contract_version()` 讀錯鍵、回空字串=已核。只 bump 正本鍵,`declared` 仍假——看起來 bump 了,live 仍 legacy。形狀不重開;同動與讀鍵怎麼修仍帶期限。
  - 結論:CONFIRMED 2.1.0 ≠ cut(Q26);live reader 回 `""`(Q27)。Q16 同動與 Q17 怎麼修仍 `[~]` 過期擋 G2。
- ⚠️ Q:graph 是改 hop、還是只改用字?為什麼本討論不准默選?
  ...
  - 推理:brief F3「做」= guide／STATUS 用語。F2 Out of Scope 把 **graph 用語**捆進 F3。現場新 slug 等人是因為 graph 預設進 `N7-g1`／`N6-g2`。四選項(改 graph／宣告 2.1.0 dual-read／兩者／只翻 coordinator 留 N7-g1)任選一邊都會偷做或空切。本討論不選。
  - 結論:OPEN Q21 四選項;Decision 必須對帳,不得默選。
...
- ⚠️ Q:F3 完成為什麼不能等於「函式真了」或「指南寫五站」?
  - 事實:docs/dev/five-station-f2/4-spec.md:L826-L830 notes/design/five-station-simplify-brief-v3.md:L165-L168 scripts/check-gate-tokens.sh:L44-L60 docs/dev/five-station-f2/7-review.md:L445 scripts/test-five-station-f2.sh:L1-L11
  - 推理:F2 已證「檔在／單路綠 ≠ 完」;D-1 再釘檔案地圖列 ≠ cut。F3 具名 hollow=`f3_cut_happened==True`／檔在／只 F2 綠。本 hop 電池綠只證明地板還在,不是 F3 完。
  - 結論:CONFIRMED G-success-1／Q19;三路可獨立紅才算完;F2 綠是地板。
...
- ⚠️ Q:本 hop 有沒有偷選落點、偷改 graph、或發明 G1?(盲點)
  - 事實:notes/design/five-station-simplify-brief-v3.md:L173 docs/dev/STATUS.md:L10-L13 docs/dev/STATUS.md:L32-L34 docs/dev/HISTORY.md:L725-L729
  - 推理:Requested solution 只列候選。Q20／Q21／Q22 維持 `[>]`。4A 形狀已 `[x]`,不重開。#359 已開看板,本 PR 仍不改 STATUS。不選 attestation 格、不改 graph、不 bump 2.1.0。
