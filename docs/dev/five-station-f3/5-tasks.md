...
> Winner B 獨立全稿＋standing soft-fix（不換 winner；吸收 A 的 ATTEST 五切片／READ-SEAM 三刀／PRE-AND 三缺／活樹 cut 延到 T-10；C 的 `--probe` exit；R2/R3 用語交付改掛 T-6、T-5 刪發明通過字）。不落地實作碼。
...
|---|---|---|---|
| T-1 | ATTEST-VISIBLE `--slot ok` | `new5/attest-visible/ok/docs/dev/f3-cut-attestation.json`（`who`／`when` 非空、`which_condition`＝`f3-cut`） | 綠：人指得到三槽；`f3_cut_happened` 回真；只讀這份 fixture 檔，不是活樹 cut |
| T-1 | ATTEST-VISIBLE `--slot missing` | `new5/attest-visible/missing/`（**無**該 JSON） | 綠：讀端 `False`；不得因契約／guide 已寫五站而回真 |
| T-1 | ATTEST-VISIBLE `--slot empty` | `new5/attest-visible/empty-who/docs/dev/f3-cut-attestation.json`（`who`＝`""` 或缺 `when`） | 綠：讀端 `False` |
| T-1 | ATTEST-VISIBLE `--slot readonly` | 同上 `ok/`；呼叫前後比位元組 | 綠：函式不建／不覆寫／不刪該檔 |
| T-1 | ATTEST-VISIBLE `--slot sibling-reject` | `new5/attest-visible/sibling-bool.md`（cut 寫進契約 JSON 兄弟鍵） | 綠：該寫法被拒；SoT 仍是獨立三槽檔 |
| T-2 | ATTEST-SILENT-RED | `new5/inject-silent-true.md` | **注入** 字面 `return True`、無合格三槽，卻稱已切 → **該格紅** |
| T-3 | READ-SEAM（舊 reader 紅） | `new5/read-seam/devflow-contract.json`（正本鍵已 `2.1.0`；reader 仍讀 `version`／`contract_version`） | **注入** 只 bump 正本、reader 仍舊、卻稱已宣告 → **該格紅**；`declared` 仍假 |
| T-3 | READ-SEAM `--reader canonical-200` | `new5/read-seam/canonical-200.json`（只有 `devflow_contract_version`＝`2.0.0`） | 綠：回 `2.0.0` |
...
- Files: scripts/test-five-station-f3.sh, scripts/five_station_f2.py, scripts/five_station_f3.py, scripts/fixtures/five-station-f3/new5/attest-visible/ok/, scripts/fixtures/five-station-f3/new5/attest-visible/missing/, scripts/fixtures/five-station-f3/new5/attest-visible/empty-who/, scripts/fixtures/five-station-f3/new5/attest-visible/sibling-bool.md
- Verify: `test -f scripts/fixtures/five-station-f3/new5/attest-visible/ok/docs/dev/f3-cut-attestation.json && test ! -e scripts/fixtures/five-station-f3/new5/attest-visible/missing/docs/dev/f3-cut-attestation.json && test -f scripts/fixtures/five-station-f3/new5/attest-visible/empty-who/docs/dev/f3-cut-attestation.json && test -f scripts/fixtures/five-station-f3/new5/attest-visible/sibling-bool.md && n=$( { bash scripts/test-five-station-f3.sh --case ATTEST-VISIBLE --slot ok -v; bash scripts/test-five-station-f3.sh --case ATTEST-VISIBLE --slot missing -v; bash scripts/test-five-station-f3.sh --case ATTEST-VISIBLE --slot empty -v; bash scripts/test-five-station-f3.sh --case ATTEST-VISIBLE --slot readonly -v; bash scripts/test-five-station-f3.sh --case ATTEST-VISIBLE --slot sibling-reject -v; } 2>&1 | grep -c '^=== CASE') && test "$n" -ge 5 && bash scripts/test-five-station-f3.sh --case ATTEST-VISIBLE --slot ok && bash scripts/test-five-station-f3.sh --case ATTEST-VISIBLE --slot missing && bash scripts/test-five-station-f3.sh --case ATTEST-VISIBLE --slot empty && bash scripts/test-five-station-f3.sh --case ATTEST-VISIBLE --slot readonly && bash scripts/test-five-station-f3.sh --case ATTEST-VISIBLE --slot sibling-reject`
- Blocked-by: —
- Risk: high
- Intent: 日常要宣稱「F3 已切」，人必須打得開一份獨立 JSON，指得到誰、何時、切的是哪個 cut 位元；系統問「切了沒」只看這份檔，檔不在或任一格空就當沒切。五切片各自可數：`--slot ok|missing|empty|readonly|sibling-reject` 合計 `-ge 5`。`--slot ok` 的 `which_condition`＝`f3-cut`（只命名 cut 位元，不是三前置 AND，不是唯一戳記 `F3-cut-happened`）。改的是 cut 讀端與 `new5/attest-visible/` 四棵具名 fixture，**不是**寫活樹 `docs/dev/f3-cut-attestation.json`（活樹 cut 屬 T-10）。不會變成函式自己寫檔、不會把宣告／in-flight 捆進 `which_condition`、不會在本 T 改 graph 或 doctor。
- Boundaries: 准改模組＝`f3_cut_happened()` 讀端＋電池入口骨架＋上列具名 NEW5 fixture（ok／missing／empty-who／sibling-bool）。Data owner＝寫入 fixture 三槽檔的人類。Interface＝三槽非空 → True；缺檔／空槽 → False；呼叫前後檔位元組不變。`which_condition` 只命名 cut 位元（例 `f3-cut`），禁止三前置 AND，禁止唯一合法戳記 `F3-cut-happened`。Forbidden＝契約兄弟布林、silent `return True`（屬 T-2）、改 doctor／graph、拿本目錄當 NEW5、本 T 評 hop、本 T 寫活樹 `docs/dev/f3-cut-attestation.json`（活樹 cut 屬 T-10；本 T Files 不含該活檔）。Test seam＝換 fixture 根可注入缺檔／空槽／兄弟鍵。本 T 不掛 F2 綠當完成。

## T-2 注入 silent True 與只用字當刀讓該格紅
- [ ] 未完成
- Covers: R-1 / S-1.3, S-1.6
- Files: scripts/test-five-station-f3.sh, scripts/five_station_f2.py, scripts/five_station_f3.py, scripts/fixtures/five-station-f3/new5/inject-silent-true.md
...
- Blocked-by: T-1
- Intent: 日常有人只把函式改成永遠真、或拿 blame／看板當刀，卻說已經切了——電池這一格必須自己紅，不能靠「函式回真」混過去。改的是空切紅格，不是改 STATUS 正本、不是把用語寫進 SoT、**不是本 T 交指南用字**（七站單行→五站用語與 GRAPH-WORD-NE 同掛 T-6）。不會變成 GRAPH-WORD-NE（那格咬 graph 仍停＋用語交付，屬 T-6）、不會在本 T 碰契約 bump。
- Boundaries: 准改模組＝電池 ATTEST-SILENT-RED 跑者＋上列注入稿。Forbidden＝本 feature branch 改 `STATUS.md`、把 blame／看板當 `f3_cut_happened` 真、把「函式回真」記成本格綠、本 T 改 `guides/guide-dev-flow.html`（用語交付屬 T-6）、改 graph（屬 T-5／T-6）。本 T 結束時 `git diff --name-only` 對 `docs/dev/STATUS.md` 必須為空。Test seam＝無合格三槽＋字面 `return True`。

## T-3 讓版本讀端只咬正本鍵且錯讀該格紅
- [ ] 未完成
- Covers: R-2 / S-2.1, S-2.2, S-2.3
- Files: scripts/test-five-station-f3.sh, scripts/five_station_f2.py, scripts/five_station_f3.py, scripts/fixtures/five-station-f3/new5/read-seam/devflow-contract.json, scripts/fixtures/five-station-f3/new5/read-seam/canonical-200.json, scripts/fixtures/five-station-f3/new5/read-seam/canonical-210.json
...
- Blocked-by: T-2
- Risk: high
- Intent: 日常問「現在契約幾點」，必須只看 `devflow_contract_version`。三切片各自可數（`-ge 3`，不是 `-ge 1`＋原始碼 grep）：無旗標＝注入舊 reader（正本已 `2.1.0` 仍回空或不以 `2.1` 開頭）→該格紅、`declared` 仍假；`--reader canonical-200`＝只讀正本回 `2.0.0`；`--reader canonical-210`＝回傳以 `2.1` 開頭且 `declared` 真、cut 仍獨立為假。改的是讀鍵縫，不是 bump 活樹契約（bump 屬 T-7）、不是把 2.1.0 當成 cut。不會變成 fallback 雙讀、不會在本 T 切 hops。
- Boundaries: 准改模組＝`contract_version()` 讀端（`five_station_f2.py` 與／或 `five_station_f3.py`）＋上列三份 read-seam fixture。Interface＝只讀正本鍵；正本以 `2.1` 開頭 → `declared` 真；cut 位元仍獨立。Forbidden＝fallback 錯鍵、本 T bump 活樹 `devflow-contract.json`（同刀 bump 屬 T-7）、把 `declared` 真寫成 cut 真、改 doctor、只用原始碼 grep 冒充三切片。現況縫＝`five_station_f2.py` 讀錯鍵回空字串。Test seam＝假契約檔三份（舊 reader／canonical-200／canonical-210）。

## T-4 讓路線閘三前置缺一就舊 7 且理由咬得到缺哪一條
- [ ] 未完成
- Covers: R-2 / S-2.4, S-2.5, S-2.6, S-2.7
- Files: scripts/test-five-station-f3.sh, scripts/five_station_f2.py, scripts/five_station_f3.py, scripts/fixtures/five-station-f3/new5/pre-210-ne-cut/, scripts/fixtures/five-station-f3/new5/pre-and/no-declared/, scripts/fixtures/five-station-f3/new5/pre-and/in-flight/, scripts/fixtures/five-station-f3/new5/pre-and/no-cut/, scripts/fixtures/five-station-f3/new5/pre-hops-200/
- Verify: `test -d scripts/fixtures/five-station-f3/new5/pre-210-ne-cut && test -d scripts/fixtures/five-station-f3/new5/pre-and/no-declared && test -d scripts/fixtures/five-station-f3/new5/pre-and/in-flight && test -d scripts/fixtures/five-station-f3/new5/pre-and/no-cut && test -d scripts/fixtures/five-station-f3/new5/pre-hops-200 && n=$( { bash scripts/test-five-station-f3.sh --case PRE-210-NE-CUT -v; bash scripts/test-five-station-f3.sh --case PRE-AND --missing declared -v; bash scripts/test-five-station-f3.sh --case PRE-AND --missing in-flight -v; bash scripts/test-five-station-f3.sh --case PRE-AND --missing cut -v; bash scripts/test-five-station-f3.sh --case PRE-HOPS-200 -v; } 2>&1 | grep -c '^=== CASE') && test "$n" -ge 5 && a=$(bash scripts/test-five-station-f3.sh --case PRE-210-NE-CUT -v 2>&1) && echo "$a" | grep -q 'F3 cut 未發生' && echo "$a" | grep -qv '已宣告所以切了' && echo "$a" | grep -qv 'doctor 已綠所以可 hop' && d=$(bash scripts/test-five-station-f3.sh --case PRE-AND --missing declared -v 2>&1) && echo "$d" | grep -E -q '路線未宣告|仍舊 7' && i=$(bash scripts/test-five-station-f3.sh --case PRE-AND --missing in-flight -v 2>&1) && echo "$i" | grep -q 'in-flight' && c=$(bash scripts/test-five-station-f3.sh --case PRE-AND --missing cut -v 2>&1) && echo "$c" | grep -q 'F3 cut 未發生' && echo "$c" | grep -qv '已宣告所以切了' && h=$(bash scripts/test-five-station-f3.sh --case PRE-HOPS-200 -v 2>&1) && echo "$h" | grep -q 'SLOT-REJECT' && echo "$h" | grep -qv 'marketplace' && bash scripts/test-five-station-f3.sh --case PRE-210-NE-CUT && bash scripts/test-five-station-f3.sh --case PRE-AND --missing declared && bash scripts/test-five-station-f3.sh --case PRE-AND --missing in-flight && bash scripts/test-five-station-f3.sh --case PRE-AND --missing cut && bash scripts/test-five-station-f3.sh --case PRE-HOPS-200`
- Blocked-by: T-3
- Risk: high
- Intent: 日常契約已經 2.1.0、目錄也沒有舊站檔，但 cut 紀錄沒寫，求五站必須被擋，理由要看得見「F3 cut 未發生」，不准寫成「已宣告所以切了」。PRE-AND 三缺各自可數（`--missing declared|in-flight|cut`），整塊 PRE 五棵子樹 `-ge 5`：少宣告 →「路線未宣告／仍舊 7」；少 ¬in-flight → 理由含 `in-flight`；少 cut →「F3 cut 未發生」。有人契約還停在 2.0.0 就把 hops 預設當五站，必須 SLOT-REJECT、線不准改，marketplace／cache 換碼也不算。改的是路線閘三個布林與拒因字面，不是改 graph 檔（條件邊屬 T-5）、不是 bump 活樹、不是寫活樹 cut。不會變成 2.1.0 單獨冒充 cut、不會把 doctor 綠寫進理由。
- Boundaries: 准改模組＝coordinator `allow_legacy()`／`refuse_hop_reason()`＋上列五棵具名 fixture 子樹。Interface＝三前置 AND；缺一 → legacy；PRE-210-NE-CUT 理由**必須**含字面 `F3 cut 未發生`、**禁止**含「已宣告所以切了」；PRE-AND 三刀各自理由對得上缺的那一條；PRE-HOPS-200 ＝ SLOT-REJECT 且路線仍舊 7。Forbidden＝marketplace／cache 當第四前置、doctor 綠當路條、本 T 改 `graph.yaml`（未宣告就跳過＝S-2.6／S-3.2 紅）、本 T bump 活樹契約、本 T 寫活樹 cut。S-2.7：條件邊可先落地，但跳過側在 `declared` 假時不得生效；無條件改 hops 預設且契約仍 2.0.0＝本 T PRE-HOPS-200 紅。Error seam＝拒因字面可稽核。本 T 不測 cut 後真的跳過例行閘（屬 T-5）。
...
- Blocked-by: T-5
- Intent: 日常有人指南已經寫「五站」、但 Stage 2 預設路仍把人送進 `N7-g1`，卻標 F3 成功——GRAPH-WORD-NE 必須獨立紅，用語與行為分開記帳。同一刀把指南七站單行改成五站用語（交付物；**不在 T-2 Files**），但改完仍不是成功的充分條件。`N7-g1.md`／`N6-g2.md` 與 G1／G2／`ACCEPTED` token 還在，token 牙仍綠。改的是指南用字＋這格注入測法，不是刪閘節點來「看起來像切了」。不會變成 HOLLOW-WORD（那格咬「只用字當 F3 綠」、屬 T-10）、不會把「coordinator 拒 hop」記成本格綠。
- Boundaries: 准改模組＝`guides/guide-dev-flow.html` 七站單行＋ GRAPH-WORD-NE 注入稿＋電池該格。guide 是交付物 ≠ SoT ≠ 本格綠條件。Test seam＝guide 已五站用字 ∧ Stage 2 預設仍進 `N7-g1` ∧ 有人標成功 → 該格紅。Forbidden＝刪節點檔、刪 token、把拒 hop 記成本格綠、把本格與 HOLLOW-WORD 對調、把用語交付掛回 T-2。token 牙只呼叫 `scripts/check-gate-tokens.sh`，**不准列入 Files、不准改**。本 T 不測 hollow 用字格（屬 T-10）。
...
- Files: scripts/test-five-station-f3.sh, scripts/five_station_f2.py, scripts/five_station_f3.py, scripts/check-five-station-f3.sh, docs/dev/f3-cut-attestation.json, docs/dev/five-station-f3/5-tasks.md, docs/dev/five-station-f3/5-tasks.html, docs/dev/five-station-f3/6-implementation-notes.md, docs/dev/five-station-f3/7-review.md, scripts/fixtures/five-station-f3/new5/inject-hollow-true.md, scripts/fixtures/five-station-f3/new5/inject-hollow-files.md, scripts/fixtures/five-station-f3/new5/inject-hollow-f2.md, scripts/fixtures/five-station-f3/new5/inject-hollow-word.md, scripts/fixtures/five-station-f3/new5/inject-hollow-two-script.md, scripts/fixtures/five-station-f3/new5/html-only/, scripts/fixtures/five-station-f3/new5/inject-token-del.md, scripts/fixtures/five-station-f3/old7/
- Verify: `test -d scripts/fixtures/five-station-f3/new5 && test -d scripts/fixtures/five-station-f3/old7 && n=$(bash scripts/test-five-station-f3.sh -v 2>&1 | grep -c '^=== CASE') && test "$n" -ge 25 && bash scripts/test-five-station-f3.sh && bash scripts/test-five-station-f3.sh --help 2>&1 | grep -q -- '--only' && bash scripts/test-five-station-f3.sh --help 2>&1 | grep -q -- '--probe' && test "$(bash scripts/test-five-station-f3.sh --only new5 >/dev/null 2>&1; echo $?)" -eq 3 && test "$(bash scripts/test-five-station-f3.sh --only old7 >/dev/null 2>&1; echo $?)" -eq 3 && test "$(bash scripts/test-five-station-f3.sh --only token >/dev/null 2>&1; echo $?)" -eq 3 && test "$(bash scripts/test-five-station-f3.sh --probe hollow-true >/dev/null 2>&1; echo $?)" -eq 3 && test "$(bash scripts/test-five-station-f3.sh --probe hollow-files >/dev/null 2>&1; echo $?)" -eq 3 && test "$(bash scripts/test-five-station-f3.sh --probe hollow-f2 >/dev/null 2>&1; echo $?)" -eq 3 && test "$(bash scripts/test-five-station-f3.sh --probe hollow-word >/dev/null 2>&1; echo $?)" -eq 3 && test "$(bash scripts/test-five-station-f3.sh --probe two-script >/dev/null 2>&1; echo $?)" -eq 3 && test "$(bash scripts/test-five-station-f3.sh --probe polarity >/dev/null 2>&1; echo $?)" -eq 1 && test "$(bash scripts/test-five-station-f3.sh --not-a-real-flag >/dev/null 2>&1; echo $?)" -eq 2 && t=$(bash scripts/test-five-station-f3.sh --case TOKEN-KEEP --case TOKEN-DEL-RED --case F3-F2-REGRESS -v 2>&1) && echo "$t" | grep -c '^=== CASE' | awk '{exit !($1>=3)}' && h=$(bash scripts/test-five-station-f3.sh --group hollow -v 2>&1) && echo "$h" | grep -c '^=== CASE' | awk '{exit !($1>=6)}' && echo "$h" | grep -q 'HOLLOW-TRUE' && echo "$h" | grep -q 'HOLLOW-FILES' && echo "$h" | grep -q 'HOLLOW-F2' && echo "$h" | grep -q 'HOLLOW-WORD' && echo "$h" | grep -q 'HOLLOW-TWO-SCRIPT' && echo "$h" | grep -q 'HOLLOW-HTML-NE-GWT' && echo "$h" | grep -E -qv 'HOLLOW-OK|NEW5-MKTG' && bash scripts/test-five-station-f2.sh && bash scripts/check-gate-tokens.sh`
- Blocked-by: T-9
- Risk: high
- Intent: 日常宣稱 F3 完，必須是**同一支** `scripts/test-five-station-f3.sh` 在同一 process 把 NEW5、OLD7、TOKEN 三組都跑完才 exit 0。缺一路、跳過一路、入口只轉呼叫 F2 電池、只證明函式真、只證明檔在、只用字、兩支腳本各綠一次、只 F2 綠 → 整電池非 0。`--only new5`／`old7`／`token` 必須是 `--help` 裡的一等旗標，各須 exit **3**。`--probe hollow-true|hollow-files|hollow-f2|hollow-word|two-script` 各須 exit **3**；`--probe polarity` 須 exit **1**；未知旗標須 exit **2**，不得被「非 0」誤當 hollow 綠。`--group hollow` 仍 `-ge 6` 且六個官方名都在。25 個官方 CASE 名都在，一格都不能少。本 T 才寫活樹 `docs/dev/f3-cut-attestation.json`（三槽；`which_condition`＝`f3-cut`）＋本 slug `5`／`6`／`7`（html 可）。本 T 具名收口格＝TOKEN-KEEP、TOKEN-DEL-RED、HOLLOW-TRUE、HOLLOW-FILES、HOLLOW-F2、HOLLOW-WORD、HOLLOW-TWO-SCRIPT、HOLLOW-HTML-NE-GWT、F3-F2-REGRESS。HOLLOW 六格用具名 fixture，禁 `hollow-*.md` 萬用字。HOLLOW-HTML-NE-GWT fixture＝`new5/html-only/`（**不是** `old7/html-only/`）。F3-F2-REGRESS 綠是地板，**不是**第四條 IFF。TOKEN-KEEP＝token 牙仍綠；TOKEN-DEL-RED＝注入刪 token 卻標成功必須紅。Files 聯集不超出准許清單；`_templates/`、doctor 握手、STATUS／HISTORY、本目錄當 NEW5、刪 token、重開 F2 park 行數＝0。不准重開已過的 Stage 4 頂欄、不准把 Q21／Q22／Q23 標可選。改的是收口對照與活樹 cut，不是順便發明 G3 PASS，也不是掃 4-spec／5-tasks markdown 當綠。
- Boundaries: 准改模組＝電池入口收口＋活樹 `docs/dev/f3-cut-attestation.json`＋上列具名 hollow／token fixture＋可選 check＋本 slug 5／6／7。Interface＝單一 process 三組都跑完才 0；F2 腳本不是入口；可選 `check-five-station-f3.sh` 不是第四路。`--only` 一等旗標各 exit 3；`--probe` 五支 hollow exit 3；`--probe polarity` exit 1；用法／未知旗標＝exit 2。`--group hollow` 名詞只准官方 25 名（HOLLOW-TRUE／FILES／F2／WORD／TWO-SCRIPT／HTML-NE-GWT），禁 `HOLLOW-OK`／`NEW5-MKTG`。HOLLOW-HTML-NE-GWT 根＝`new5/html-only/`，禁 `old7/html-only/`。Forbidden＝F2 綠寫進 IFF、兩支腳本各綠當同一電池、僅 html 當 GWT、減 Decision 原 20 列、Files 超出 S-8.2、用 python 掃 markdown 當 Stage 6 綠、用未知旗標非 0 冒充 hollow、`hollow-*.md` 萬用字、改 STATUS／HISTORY、改 4-spec 頂欄（S-8.1）、重開 F2 park（S-8.6）、發明活五站名字、T-7 就寫活樹 cut。Diff Budget 0 區塊命中＝本條紅。F2／token 腳本只呼叫、不進 Files。本 T Verify 是 SC-BATTERY 全入口（`-ge 25`）＋`--only`／`--probe`／`--group hollow`＋F2 地板＋token 牙；cut／讀鍵／閘／graph／doctor／紅格已由 T-1…T-9 加厚。
...
| standing soft-fix：T-1 五切片 `--slot`；活樹 cut 不進 T-1 Files | A：缺檔／空槽／只讀／兄弟鍵必須各自可數；T-7 bump 時 cut 仍假＝合法 PRE-210-NE-CUT | 4-spec S-1.1／S-1.2／S-2.4；A T-1／T-10 | T-1 寫活樹 cut。棄：PRE-210-NE-CUT 假綠 |
| standing soft-fix：T-3 READ-SEAM 三切片 `-ge 3` | A：舊 reader 紅／canonical-200／canonical-210 各自可數，不是 `-ge 1`＋原始碼 grep | 4-spec S-2.1／S-2.2／S-2.3 | 只 grep `blob.get`。棄：假綠 |
| standing soft-fix：T-4 PRE 五棵子樹 `-ge 5`；保留 B 拒因字面 | A：PRE-AND 三缺獨立可數；B：`F3 cut 未發生`、禁「已宣告所以切了」 | 4-spec S-2.4…S-2.7 | 一條 PRE-AND 不含三缺。棄：分不清哪一前置沒測 |
| standing soft-fix：T-10 `--probe` exit 3／polarity 1／未知 2 | C 探針利；保留 B `--only` exit 3 與 `--group hollow -ge 6` 具名鎖 | 4-spec S-5.3…S-5.11 | `! bash --only` 靠未知旗標非 0。棄：假綠 |
| standing soft-fix：HOLLOW-HTML-NE-GWT＝`new5/html-only/` | A 表；C 的 `old7/html-only/` 會把「僅 html」誤種在 OLD7 根 | 4-spec S-7.1／S-5.11 | `old7/html-only/`。棄：html 被當 in-flight 鄰居 |
| standing soft-fix：T-5 刪發明通過字；用語交付掛 T-6 | R3：只留無例行 `N7-g1`／`N6-g2`、官方名、禁 `GRAPH-AGREE`。R2/R3：guide 不進 T-2 Files | 4-spec S-3.3／S-3.5 | `allow_legacy=false\|why=five`；T-2 交指南。棄：發明通過字／用語錯掛 |