...
### 7.1.5 原本沒測的那一半 —— 2026-08-19 已封閉

**透過真正的安裝流程之後在全新 session 裡叫這兩個型別**，原本是缺口，v3.9.0 發版當天補上：
從市集裝進 `~/.claude/plugins/cache/dev-flow/dev-flow/3.9.0`（安裝紀錄與市集來源見
`agents/devflow-reviewer.md` 的「正式安裝那條路」節），`/reload-plugins` 之後
**兩支都真的被叫起來並回話** —— reviewer 判 `PASS`、adviser 判 `CONTINUE`，
且 adviser 順手抓到 `scripts/check-py-floor.sh` 一個真缺陷（成功訊息寫死宣告下限、
沒報實際用到的版本，已修）。

**這次拿不到、要如實記的**：兩支都回報那些寫檔／跑指令／搜尋的工具**根本不在清單裡**，
連呼叫都發不出去，所以**本輪產不出逐字的拒絕訊息**；上一輪（臨時載入）才有。
探針機制不同，結論相同。

**引用本節時不要升級成「這能保證叫得出來」** —— 現有證據是「臨時載入兩次 + 正式安裝一次，
三次都叫出來了」，換平台版本或換作業系統都要重測。
...
| **6** | §7-3a／3b 兩個探針（記錄用），結果寫成 selftest fixture | 不阻斷前面五步，但要留檔給第二輪用。**✅ 2026-08-19 已結案** —— 重跑 + 第二人獨立複核，結論見 §7.1；原本缺的那一半（正式安裝路徑）也在 v3.9.0 發版當天補測完成，見 §7.1.5 |
