...
> 正本（Decision D / 4-spec R-3）。人／agent **先選** `family`，再跑對的產器。
> 閘不黑盒猜列。缺欄或選錯 → `DIAGIR_FAMILY`。
> 不是 mermaid，不是一支 API 吃五族，沒有第六列 hosted／WYSIWYG。
...
| behavior-flow | gate-twin 行為流程；樹狀改 WARNING+`<pre>` | 樹收成單盒 vbox；mermaid | build-gate-twin.py | vbox-fig-contract（twin 收口） |
...
1. 打開本表，勾一個 `id`。
2. 信封只准兩個請求鍵：`family`（上列 id 之一）+ `payload`。
3. 正式入口：`python3 scripts/diagir.py deliver ENVELOPE.json --out TARGET`。
4. 印表：`python3 scripts/diagir.py route`。
