...
**E7 的 4-spec 正本(1.3.2)**:Gauntlet 預設讀 sibling `4-spec.md` 的 Verification
Profile Required layers;未 pass → E7 紅。`--require-layer` 只能加嚴,不能把
Required 拿掉。漏帶旗標不再 fail-open。`--review-file` 找不到 Profile(無 sibling
或檔內無該節)→ E7 fail-closed,不得退回 1.2.0。`--review-file` 時 Required layers
欄必須在(可寫「無」/none;缺欄不得當零層放行)。`--review-file` 的 `--profile`
只准 sibling 或同一 `docs/dev/<slug>/` 的 4-spec,不得用別份 feature 覆寫。
已列入 Evidence 且非 n-a 的 Conditional 層同樣必須 pass。
