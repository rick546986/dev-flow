...
- 掃頁 `#scan-log`：四欄表外包既有 `.tablewrap`，與 md 同形。S10 仍六件。窄 viewport 用橫向捲動，不堆疊。
...
- `OPEN` 是 session 狀態（`memory/agentmem/session.py`），不是上表任何一軸。Interview Log 結論欄的 `CONFIRMED`／`NEEDS_VERIFICATION`／`OPEN`（§6.3）是條目狀態詞，與記憶／session 三軸**同形不同義，不得互借**：Log 的 `CONFIRMED` 不是 `known_knowledge` 的 `CONFIRMED`；Log 的 `NEEDS_VERIFICATION` 不是 `ask()` 的 `retrieval_status`；Log 的 `OPEN` 不是 session 的 `OPEN`。
...
- 子項下的續行併入上一欄（以空白接），不靜默丟。
- 事實欄只放 Context 已列出處（同一路徑，行段可更窄）；不另造無出處斷言。每條事實欄必須至少一個 `path:L` citation。
- 結論欄前綴**只允許**這三個：`CONFIRMED`｜`NEEDS_VERIFICATION`｜`OPEN`。每個前綴後面必須接一個空白，再接一句非空結論（regex 意圖：`^(CONFIRMED|NEEDS_VERIFICATION|OPEN)\s+\S`）。只有前綴、或 `CONFIRMEDx` 這類無邊界黏字，都不合格。這是 Log 條目狀態，與記憶／session 軸同形不同義，不得互借：不是 Open Questions 的 `已解`／`假設`／`移交`，也不是 `known_knowledge` 的 `CONFIRMED` 或 session 的 `OPEN`。不得為了填滿四段硬下結論，寫 `NEEDS_VERIFICATION` 或 `OPEN` 加一句待驗證／未決說明即可。
...
- 上限從「八行」改為「八條」。超過八條 `ValueError`，不准靜默截斷。

## 7. Html 形狀（只動掃頁 `#scan-log`）

S10 六件不變：摘要卡／現況圖／人表／題目／驗收表／問答摘要。

`#scan-log` 仍是預設摺著的 `<details>`，`<summary>` 仍含「問答摘要」，內文從逐條 `<p>` 改為一張四欄 `<table>`（Q／事實／推理／結論），外包既有 `.tablewrap`。`check-devtalk-fig-graph.sh` 守摺疊／摘要／四欄 `<th>`／`.tablewrap`。

- wide viewport：四欄表。
- narrow viewport：本輪接受既有 `.tablewrap` 橫向捲動為可讀形態。四段堆疊不在本輪範圍、也不列為成功條件。本輪不動 html-shell；產生器輸出外包 `.tablewrap` 的四欄表即可。
- 每欄內容逐一 `esc()`，維持現狀。
- ⚠️ 以純文字前綴呈現，不加 class。
...
- 語意候選仍走 propose → 人 confirm → end 才固化。Log 結論的 `CONFIRMED` 不等於記憶的 `CONFIRMED`。
...
| 檔 | 改什麼 |
|---|---|
| `scripts/build-scan-html.py` | `parse_log` 改巢狀四段解析；四段檢查、citation 行段 ⊆、牙 3a、續行合併、>8 拒收（§10）；`#scan-log` 四欄表；移除 OQ 合成退路 |
| `scripts/check-devtalk-fig-graph.sh` | 加 `check_log_teeth`（bad fixture 進 CI）+ 生成頁 `#scan-log` 四欄／`.tablewrap` 守衛。既有 hop／六件規則不動 |
...
| `skills/dev-talk/nodes/S8-review.md` | 第④條「三段」改「四段」，加「結論前綴只允許 CONFIRMED｜NEEDS_VERIFICATION｜OPEN，且後接空白與非空句子」「事實欄 ⊆ Context」 |
...
2. 結論欄前綴只允許 `CONFIRMED`｜`NEEDS_VERIFICATION`｜`OPEN`，且必須符合 `^(CONFIRMED|NEEDS_VERIFICATION|OPEN)\s+\S`，否則 `ValueError`。
3. 事實欄每個 citation（`path:L起` 或 `path:L起-L迄`）必須 path 相同且行段 ⊆ Context 已列 citation 行段，否則 `ValueError`。比對前剝 Context HTML 註解；路徑 token 兩端剝 `` ` ' " （ ） ( ) 「 」 ``。產生器不讀產品檔案系統。
3a. 事實欄必須至少一個 `path:L` citation，否則 `ValueError`。
4. 舊單行 `|` 格式、空 Log → `ValueError`。
5. 超過八條 → `ValueError("Interview Log 上限八條")`，不准 `entries[:8]` 靜默截斷。
...
2. 牙 1–4／3a／5 有對應 fixture（good 一份、每條牙一份 bad），且 `check-devtalk-fig-graph.sh` 的 `check_log_teeth` 會跑它們。
3. 掃頁 `#scan-log` 為四欄表，外包既有 `.tablewrap`；`<details>` 預設摺著、`<summary>` 含「問答摘要」。wide = 四欄表；narrow = `.tablewrap` 橫向捲動即為本輪接受的可讀形態。不要求四段堆疊。
...
- 結論前綴只允許 `CONFIRMED`｜`NEEDS_VERIFICATION`｜`OPEN`，後接空白與非空句子（§6.3）。Open Questions 仍是 `已解`／`假設`／`移交`。
- 窄 viewport 接受 `.tablewrap` 橫向捲動；四段堆疊不在本輪範圍、不是成功條件（§7）。
