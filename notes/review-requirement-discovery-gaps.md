...
> **裁決日期：2026-09-12。** 下方分析全文保留，不改寫成規格。
>
> ## 本檔仍不是派工單
>
> Owner Call 已逐條落檔。**實作 = 之後另開的 feature**，不得把本檔九條混進
> `integration-before-verdict` 或本 PR 實作。未另開 feature、未過該 feature 的
> G1/G2 前，不得改 `dev-talk` 核心契約、Stage 1–4 模板與範例。
>
> 2026-08-18 暫緩理由仍成立（設計是否恰當只能靠真跑；動核心契約風險不對稱）。
> 裁決只關閉「裁不裁」，不授權本檔直接施工。

---

> 分級沿用採用回饋的語意：**A**＝可能讓流程全綠、卻解錯問題；**B**＝不一定立刻解錯，
> 但會讓證據不足或人機互動風險到後段才暴露。分析段仍是候選推論；約束力只在 Owner Call。

## Owner Call（2026-09-12）

裁決人：owner（本場書面 brief）。實作落點一律「另開後續 feature」，本檔不施工。

| ID | 裁決 | 約束（摘要） | 機械守衛 |
|---|---|---|---|
| A-1 | **DO** | Goals 只寫人的結果／工作狀態。使用者帶來的做法另標 `Requested solution`（候選，未定案），不得混成 Goal。驗收雛形不鎖畫面路徑、API、元件、通道。Stage 2 在原因尚可能由流程／政策／資料品質解決時，比較一個 no-build／process-only 方案；不適用則寫理由，不為湊數硬塞。 | 後續 feature 再定；可守分欄與 Stage 2 no-build 適用性判定，Goal 是否偷帶解法留給 G1。 |
| A-2 | **DO** | **發現題／事實題**禁附推薦答案。**裁決題／設計題**可附選項、差異與推薦。 | 後續 feature 再定；skill／guide 對稱措辭可靜態互釘，不假裝對話語意硬 gate。 |
| A-3 | **DO** | 主張必須連到來源，或標 `Assumption` **且**有驗證期限。使用者點頭 ≠ 來源升格。 | 後續 feature 再定；可驗欄位／枚舉，不驗「有 Evidence 字樣即可」。 |
| A-4 | **DO** | 高影響 Assumption 必寫風險與驗證期限。期限到期未驗 → **擋 G2**。 | 後續 feature 再定；high 必有 resolved 或 Owner Call、期限過期擋送審。 |
| A-5 | **LIGHT** | Human verdict 補一行：角色／場景（誰、在哪種場面按的）。不在本裁決加 Actor Coverage 全表。 | 後續 feature 再定；可驗一行非空，代表性仍是人工。 |
| A-6 | **DO** | Stage 2→4 建 disposition ledger：高影響痛點／workaround／exception 逐條標記本方案處理／刻意維持／Non-Goal／另開 slug／仍待驗證。標「本方案處理」者 Stage 4 至少落到一條 R/S。 | 後續 feature 再定；可驗非空與 addressed→R/S，高影響判定留給 reviewer。 |
| A-7 | **DO** | 出貨後必留改善回看：日期、owner、資料來源、低於何值要重開討論／回滾／另開 feature。不另造無維護者的永久檔。 | 後續 feature 再定；可檢落點與到期未回看，指標是否代表改善是人類判斷。 |
| B-1 | **DO** | 保留禁讀下游方案文件。另開 **owner 核准** 的事實型 evidence 入口：使用者提供／授權的 support、incident、analytics、SOP、去識別化 log、表單與畫面；授權後才讀。 | 後續 feature 必須改允許集合，避免「規定了但讀不到」。 |
| B-2 | **DO** | Fast lane 在進 Stage 4 **之前**做最小 interaction risk triage（下一步／權限核准／等待完成語意／角色交接／系統外動作／中斷恢復）。全否且行為已有 approved spec 才可直接 fast。 | 後續 feature 再定；命中卻無 full／mini／Owner Call 即拒。 |

未採「整包暫緩、等 full lane 觀測完再裁」。觀測實驗（STATUS Backlog 那列）仍可跑，但不阻擋本裁決。Windows 真機驗證已從本輪看板拿掉，不掛在這九條上。
...
2026-09-12 已用檔首 **Owner Call** 表落地，不再用本段空白範本。原建議列格式保留備查：

```text
- A-1: 採 / 不採 / 改案 — 理由；若採，指定落點與是否要機械守衛
```

實作不得把本區混進既有「四項已定」驗收；九條改 `dev-talk` 核心契約、Stage 1–4 模板與範例，
應各（或分批）獨立走 G1/G2，並保留現行 real-world 機制的回歸基準。
