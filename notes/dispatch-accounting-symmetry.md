...
> 　四、**採用現場實際踩到的 3 條母版缺陷**（G1/G2/G3，2026-08-17 由**兩個不同的採用專案**
> 　回報，owner 端已逐條複驗）　五、**新增「缺陷回報」skill**（把採用現場 → 母版待辦
> 　這條路徑標準化）。
> 做完之後 owner 會再驗收一次。
...
## G3（HIGH）：全形冒號的字元集打錯 —— **6 處，其中 2 處在 runtime**

第二個採用專案回報：「`check-spec-gate.sh` 的 C1 不認全形冒號 → 完整的規格書被判
『缺 13 條觀測欄』，照著補就會去編造當初沒實跑的觀測值」。

**owner 端複驗屬實，但根因比回報的更精確 —— 不是「不認全形」，是「字面打錯」。**

`scripts/check-spec-gate.sh:85`：
```python
OBS = re.compile(r"^\s*-\s*\*{0,2}\s*觀測\*{0,2}[^::\n]*[::]")
```
看起來像「全形或半形都收」，但四個冒號的字元碼實測**全部是 `0x3a`（半形）**——
字元集裡放了兩個一模一樣的半形冒號，**全形 `：`（0xff1a）根本不在裡面**。
寫的人本意是對的，只是全形半形長得幾乎一樣，肉眼看不出來。

對照組（正確寫法，`build-gate-twin.py` 的 observe pattern，字元碼
`['0xff1a','0x3a','0xff1a','0x3a']`）：五種寫法全過。

### 同型盤查（owner 端已做，**6 處**）

| 位置 | pattern | 影響 |
|---|---|---|
| `scripts/check-spec-gate.sh:85` | `觀測…[^::\n]*[::]` | **回報的那條**：全形冒號寫的觀測欄被判缺欄 → 誘導使用者編造沒實跑過的觀測值 |
| **`hooks/devflow-lib.py:688`** | `Owner[- ]Call ?例外\s*[::]` | **runtime，最嚴重**：用全形冒號寫的 Owner Call 例外偵測不到 → `lane: fast` + `Risk: high` 明明寫了例外卻被擋，**開不了工且不知道為什麼**（與 A-0/A-13 同型：規定了做不到的事） |
| **`hooks/_stage3_impl.py:52,53,54`** | `Human verdict\s*[::]` / `Verdict attestation\s*[::]` / `^human\s*[::]` | **runtime**：第 3 站的人類確認用全形冒號寫會解析不到 → 判成沒填 |
| `scripts/check-realworld.sh:167` | `Operational Context\s*[::(]` | 檢查誤判 |
| `scripts/build-gate-twin.py:753` | `re.split(r"[,。;;]", ln)` | 兩個半形 `;`，本想寫全形 `；`；影響判定格摘要的截斷點（小） |

（`[[:space:]]` 那類 POSIX 字元類是誤報，已排除。）

### 修法要求

1. **六處全改**（第 6 型的字面要求：同類一起長）。改完逐處實測全形版本。
2. **加一支機械檢查**：掃 `scripts/`、`hooks/` 的所有正則字元集，
   找「**同一個字元集裡出現重複的半形標點**」（`[::]`、`[,,]`、`[;;]`、`[((]`…）
   —— 那是「本想寫全形卻打成半形」的機械徵象。註冊進 `devflow-check`。
   **這是通解**：不做的話，下次有人再寫一個 `[::]` 又會重演，而且一樣肉眼看不出來。
   ⚠️ 要排除 POSIX 字元類（`[[:space:]]`/`[[:alpha:]]` 等）的誤報。
3. **回歸案**：至少對 `check-spec-gate.sh` 的 C1 與 `devflow-lib.py` 的 Owner Call 例外，
   各加一條「全形冒號版本必須通過」的 fixture。
4. 順帶想：模板與範例是不是該**明講**「全形半形皆可」，還是**只准半形**？
   兩種都行，但要選一個並讓機械層守住 —— 現在是「文件沒說、程式只認半形」的最糟組合。
...
G1/G2/G3 三條都是**採用專案踩到、人工轉述回母版**的。這條路徑目前靠 owner 當中繼：
現場踩到 → 口頭轉述 → 母版端驗證 → 寫進派工單。有效，但每次都要人在中間。

**這個 skill 要做的不是「開 issue」，是把那條路徑標準化。**

## owner 已裁決的設計（照做，不要改方向）

| 決定 | 內容 |
|---|---|
| 走哪條路 | **(a) 產出去識別化的回報檔，人工貼** —— **不自動開 issue** |
| 去識別化時機 | **呼叫 skill 當下就做**，不是產出後再篩 |
| 誰修 | owner 固定時間處理，**不需要自動指派或通知機制** |

⚠️ **為什麼不自動開 issue**：`dev-flow` 是 **public repo**（`isPrivate: false`，Pages 也開著）。
採用專案是公司的 —— 專案名、路徑、規格內容一旦進 public issue 就是公開的。

## 產出兩份（同一份材料，兩個用途）

1. **給 issue 貼的**：去識別化後的缺陷描述（現象／重現／影響／環境版本）
2. **給派工單貼的**：直接是 `notes/dispatch-*.md` 的條目格式
   （`檔案:行號` + 一句缺陷 + 具體失敗情境 + 建議修法方向）

這樣「回報」與「修復」用的是同一份材料，不用轉寫第二次。

## 去識別化規則：**白名單，fail-closed**

⚠️ **不要用黑名單列公司名稱** —— 那份清單本身就是洩漏，而且會進 public repo。
...
| exit code、檢查名稱、守衛輸出的**固定文字** | `❌ C1 每個 S 都有觀測欄` |
| plugin 版本、契約版本 | `3.6.1` / `2.0.0` |
...
## 去識別化要有**機械層**（owner 追加裁決）：一支 hook，每次都跑

skill 是 prompt 級 —— 會被繞過、會忘記、失效時是**靜默的**。owner 明確要求
**去識別化要變成 hook，每次都執行**。方向正確（同 X-5 把模型分層從 prompt 變機械層），
但**必須先講清楚 hook 做得到哪一半**：

### hook 抓得到的（**機械可判**，這部分做成 hook）

| 特徵 | 例 |
|---|---|
| 絕對路徑 | `/Users/…`、`/home/…`、`C:\…`、`~/dev/…` |
| git SHA | 7 或 40 位 hex（單獨成詞） |
| email | `\S+@\S+\.\w+` |
| 內網 IP / 主機名 | `192.168.*`、`10.*`、`*.local` |
| **不存在於母版 repo 的檔案路徑** | 回報檔提到 `src/foo/bar.go` 而母版沒有這個檔 → 那是採用專案的 |
| 分支名 | `feature/*`、`fix/*` 之類的具體分支 |

**最後那條是這支 hook 最有價值的判準** —— 它不需要知道任何公司名稱，
只要問「這個路徑在母版存在嗎」。不存在 = 來自採用現場 = 要人確認。

### hook 抓不到的（**語意判斷**，只能靠 skill 白名單 + 人工確認）

公司名、系統名、產品名、資料表名、業務術語 —— 這些**沒有結構特徵**。
`icryobank`、`HiStork` 這種字串，對 hook 而言就是普通英文字。

⚠️ **不得用黑名單列這些名稱來補**（前面已經寫過理由：清單本身就是洩漏，且會進 public repo）。

**所以是兩層，缺一不可**：hook 擋結構特徵（每次都跑、擋得死），
skill 的白名單 + 人工確認擋語意（可繞過，但那是最後一道人為關卡）。
**回報裡不得宣稱「hook 上了就安全了」。**

### hook 的形狀（仿 `hooks/devtalk-guard.sh`，那支是最接近的既有範本）

- 事件：`PostToolUse: Edit|Write`（寫完才掃，跟 devtalk-guard 同型）
- **路徑限定**：只掃回報檔的固定位置（**你要先定一個路徑慣例**並寫進 skill 與 README，
  例如 `.devflow/reports/*.md` —— 選什麼由你定，但必須是**單一明確的 glob**）。
  **其餘路徑一律靜默放行 `exit 0`** —— 這條錯了會變成「裝了 plugin 就不能寫任何檔」。
- 命中 → `exit 2` + stderr 印出**命中的行號與該行內容**，要求先去識別化再寫
- 比照 devtalk-guard 記一筆 observability 事件（`|| true` 包起來，
  obs 寫入失敗絕不得影響 deny/放行判定）

### 三個必須避開的既有坑

1. **F2（ARG_MAX）**：新 hook **不得**用 `HOOK_INPUT=$(cat); export HOOK_INPUT`。
   用本輪修好的寫法（第一部分 F2）—— 否則大檔案下這支也會靜默失效。
2. **F1／第 7 型（不對稱記帳）**：`hooks.json` 會從 6 條變 **7 條**。
   `skills/dev-setup/SKILL.md` 的掛載數與可執行清單、`README.md`、
   `guides/guide-dev-flow.html` 的 hooks 註冊表**全部要同步**，
   而且要被 F1 那支新守衛涵蓋。**F1 先做，這支才不會變成下一個漏記的。**
3. **selftest**：正負向案都要有 —— 命中要擋、非回報路徑要放行、
   空輸入/壞 JSON 要放行（fail-open 於環境問題）。

## 產出前必須做的一步（人工那一層）

把產出的兩份**完整顯示給使用者**，並附一行：
> 「以下內容將被貼到 **public** repo，請確認沒有不該公開的東西。」

**使用者沒明確說可以，就不算完成。** hook 擋的是結構特徵，
**語意上的識別資訊只有人看得出來** —— 這一步不可省略。

## 實作要求

- 位置：`skills/<名稱>/SKILL.md`（名稱由你定，建議動詞開頭、與現有 `dev-talk`/`dev-run`/
  `dev-setup`/`dev-release` 同族）
- **description 要寫觸發詞**：讓使用者說「回報 bug」「這是母版的問題」「dev-flow 有 bug」
  之類就能觸發，不用記 skill 名
- 註冊進 `.claude-plugin/`（跟著 plugin 走，**不需要 dev-setup 散發**）
- **納入既有守衛**：`gate-consistency.sh` 若會掃 skill 清單，要一起更新；
  `check-dev-setup-discipline.sh` 那類列舉 skill 的地方同理
  —— **這正是第 7 型（不對稱記帳）的字面要求，別讓新 skill 變成下一個漏記的東西**
- README 的 skill 對照表（§8「每階段呼叫的技能」）要補一列

## 驗收

1. 拿 **G3 這條真實案例**當測試輸入（全形冒號那個），跑一次 skill，
   確認產出的兩份都能用、且**不含任何專案識別資訊**
2. 故意餵一段含絕對路徑與業務文字的輸入 → 確認 skill **停下來問**而不是自動帶入
3. 產出的「派工單條目」格式要能直接貼進 `notes/dispatch-*.md` 不用改寫

⚠️ skill 是 plugin 的一部分，**加了要發版採用專案才拿得到**（走本輪的 `/dev-release`）。
...
## 第四部分：採用現場 3 條  | id | 修法 | 回歸案落在哪 | 破壞實驗四步的輸出 |
## 第五部分：回報 skill + 去識別化 hook  | 落在哪 | hook 正負向實測 | 用 G3 當輸入的產出 |
## 待 owner 裁決(如果有)
## 六道回歸輸出(貼原文)
