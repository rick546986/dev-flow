...
| 部分 | 內容 | 為什麼是這個順序 |
|---|---|---|
| **一** | 補 6 條守衛覆蓋缺口（獨立審查 N-1~N-6） | **先做** —— 後面兩部分的改動要靠有牙的守衛保護，守衛先補好才有意義 |
| **二** | 清掉 `docs/dev/STATUS.md` Backlog 剩餘 5 條 | 那是 v3.4.0 那輪產生或收攏的真實剩餘 |
| **三** | 補驗 Backlog 14 條裡 11 條的**行為層**證據 | 上一輪的獨立審查只驗到引用層，行為層沒查 |

**第一部分的驗收標準不是「檢查變綠」，是「故意弄壞，檢查會不會紅」。**

v3.4.0 的產物本身通過了 12 個舊 bug 回歸與 12 個破壞實驗、六道回歸全綠、派工單四批
全部做完。獨立審查判 REQUEST_CHANGES 的理由只有一句：
...
正本在 `docs/dev/STATUS.md` 的 Backlog 表（每條都附了出處，**先讀出處再決定做法**）。

| 級 | 一句 | 出處 | 備註 |
|---|---|---|---|
| **B** | **Boundaries 裡被 fence 包起來的 `## T-xx` 會被 Stage 6 引擎長成幽靈任務** —— 引擎的 `parse_5_tasks` 不遮蔽 code fence | `7-review.md` 附錄 A7 H1 | **這是 twin 那邊已經修掉、引擎那邊還沒修的同一個病**。twin 已加警告現形。屬 **live runtime 改動**：`hooks/devflow-lib.py::parse_5_tasks` 要跟 `tests/parallel-stage6/contract_ref.py` 與 `hooks/selftest.sh` 一起動，缺一不可 |
| **B** | 第二個範例 feature（fast lane 或合法跳過 Stage 3），破「唯一範例自證循環」 | 4cap `O-3` 裁決 | 現在所有機械檢查都對著同一個範例跑 —— 範例本身錯了沒人知道。**工作量最大的一條，你裁決要不要這輪做**；不做要寫明理由 |
| C | `devtalk-guard.sh` 的 deny 沒寫 obs 事件，也無 selftest 覆蓋 | `notes/change-manifests/observability.md` 2026-08-15 核對註 | observability manifest 第 4 項僅部分落地 |
| C | 審查圍欄（圍欄③）期間 postbash 的 `allowed_prefix` 豁免未收緊 —— shell 改 5-tasks/6-notes 仍走既有豁免 | `adoption-findings` A-11 已採用的修法 | 圍欄③ 是這輪新加的，這是它的已知缺口 |
| C | SDC 大表、Reference App | 4cap `§7` 第 5 點收攏的低優先 deferred | 低優先，**判斷不值得做就寫明理由留在 Backlog**，不要硬做 |

**做完的移出 Backlog、沒做的留著並補上「本輪不做的理由」。不要為了清空而硬做。**

---

# 第三部分：補驗 Backlog 14 條的行為層

上一輪的獨立審查對第二批（A-1~A-6、A-11、A-12、B-1~B-6）**只驗到引用層**
（47 條 `檔案:行號` 逐條 Read，44 條正確、3 條指錯 → 就是 N-6）。

**行為層只有 3 條有第一手證據**：A-2 的兩個破壞實驗（`_s_ids_of` 退回舊 pattern、
`s_id_present` 恆 True，selftest 都紅）、A-1 的 README 標記與新守衛存在。
**其餘 11 條只有實作者自述的修法段落** —— 那正是「斷言釘在副本」的審查版。

> ⚠️ 勘誤（2026-08-17）：本段「3 條有第一手證據」逐字只點名 2 條（A-2、A-1），
> 14 − 2 = 12，正確為 **12 條**（本段「其餘 11 條」的算法沿用了上面「3 條」這個
> 筆誤，未發現內部不一致）。判定依據見
> `docs/dev/b8-gate-twin-review-ui/7-review.md` 附錄 A9。

要做的：**派一個 fresh-context sonnet，對這 11 條逐條做「弄壞會不會紅」**。
每條問同一個問題：

> 這條宣稱修好的東西，如果我把修法退回原狀，有沒有任何機械檢查會發現？
> 沒有 → 那它就是「改了但沒有守衛盯著」，要補守衛或至少列進 Backlog。

**不是要它重審一次修法對不對**，是要確認**修好的東西有沒有防回歸的東西盯著**。
結果分三類回報：①有守衛且實測會紅 ②有守衛但實測不會紅（＝假綠，要補）
③根本沒有守衛（要判斷值不值得補，不補要寫明）。
...
- `docs/dev/STATUS.md` Backlog 反映真實剩餘（第二部分做掉的移出、沒做的補上理由）
